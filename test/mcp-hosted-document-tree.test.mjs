/**
 * Hosted MCP get_document_tree tests.
 *
 * Phase 1E mirrors hosted get_note_outline access: same canister read path,
 * same auth headers, viewer-level ACL, and a body-free DocumentTree response.
 */
...
function makeCtx(overrides = {}) {
  return {
    ...
    canisterAuthSecret: 'gw-secret-tree',
    ...overrides,
  };
}
...
async function connectPair(ctx = makeCtx()) {
  ...
  const client = new Client({ name: 'hosted-document-tree', version: '0.0.1' });
...
describe('hosted MCP get_document_tree', () => {
  let mock;
  let client;
  ...
  it('lists get_document_tree as a read tool for every hosted role', async () => {
    ...
    for (const role of ['viewer', 'editor', 'evaluator', 'admin']) {
      ...
      assert.ok(tools.some((tool) => tool.name === 'get_document_tree'), `${role} can list get_document_tree`);
      await client.close();
      client = undefined;
    }
  });
  ...
  it('uses the same canister GET path and auth headers as get_note', async () => {
    mock = installFetchMock(() => ({
      ...
      json: async () => ({
        ...
        frontmatter: '{"title":"Hosted Tree","api_key":"must-not-leak"}',
        body: '# Intro\n\nBody must not leak.\n\n## Next',
      }),
      text: async () => '{}',
    }));
    ...
    assert.equal(headerGet(headers, 'Authorization'), 'Bearer tok-tree');
    assert.equal(headerGet(headers, 'X-Vault-Id'), 'vault-tree');
    assert.equal(headerGet(headers, 'X-User-Id'), 'google:owner');
    assert.equal(headerGet(headers, 'X-Gateway-Auth'), 'gw-secret-tree');
  });
  ...
  it('returns DocumentTree JSON without body, snippets, full frontmatter, or absolute paths', async () => {
    mock = installFetchMock(() => ({
      ok: true,
      status: 200,
      json: async () => ({
    ...
    assert.equal(data.title, 'Hosted Tree');
  ...
  it('does not expose document tree-specific resource URIs', async () => {
    ...
    assert.equal(resourceText.includes('document-tree'), false);
    assert.equal(resourceText.includes('get_document_tree'), false);
  });

  it('ignores unsafe upstream paths and does not leak absolute path or body', async () => {
    mock = installFetchMock(() => ({
      ok: true,
      status: 200,
      json: async () => ({
        path: '/Users/aaron/private/secret.md',
        frontmatter: '{"title":"Safe Tree"}',
    ...
    assert.deepEqual(data.root.children, [
      {
        id: 'h1-safe-heading-0001',
        level: 1,
        text: 'Safe Heading',
        children: [],
      },
