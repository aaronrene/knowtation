/**
 * Gateway REST NoteOutline tests.
 *
 * The hosted gateway route returns heading-only NoteOutline metadata while
...
const SECRET = 'gateway-note-outline-rest-test-secret-32';
const GATEWAY_AUTH_SECRET = 'gateway-note-outline-rest-gw-secret';
...
async function startGateway(canisterUrl, bridgeUrl) {
  ...
  const { app: gwApp } = await import(`${gwEntry}?gwnoteoutline=${Date.now()}-${Math.random()}`);
  return startServer(gwApp);
}

test('unit: GET /api/v1/note-outline is auth-gated before upstream fetch', async (t) => {
  const canisterCalls = [];
  ...
  const res = await fetch(`${gateway.url}/api/v1/note-outline?path=inbox/a.md`);
...
test('integration: NoteOutline REST uses active vault and effective canister user headers', async (t) => {
  const bridgeCalls = [];
  ...
  bridge.get('/api/v1/hosted-context', (req, res) => {
    ...
    res.json({
      ...
      allowed_vault_ids: ['vault-outline'],
      role: 'viewer',
    });
  });
  ...
  canister.get('/api/v1/notes/:path', (req, res) => {
    ...
    res.json({
      ...
      frontmatter: '{"title":"REST Outline","api_key":"must-not-leak"}',
      body: '# Intro\n\nBody must not leak.\n\n## Next',
    });
  });
  const canisterSrv = await startServer(canister);
  ...
  const res = await fetch(`${gateway.url}/api/v1/note-outline?path=${encodeURIComponent('inbox/hello world.md')}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      'X-Vault-Id': 'vault-outline',
    },
  });
  const body = await res.json();
  ...
  assert.equal(bridgeCalls[0].vault, 'vault-outline');
  ...
  assert.equal(canisterCalls[0].vault, 'vault-outline');
  assert.equal(canisterCalls[0].gatewayAuth, GATEWAY_AUTH_SECRET);
  assert.equal(body.schema, 'knowtation.note_outline/v1');
  assert.equal(body.path, 'inbox/hello world.md');
});

test('end-to-end: NoteOutline REST returns the body-free allowlist only', async (t) => {
  const canister = express();
  ...
  const token = signTestJwt({ sub: 'google:actor', role: 'viewer' });
  const res = await fetch(`${gateway.url}/api/v1/note-outline?path=safe.md`, {
...
test('stress: NoteOutline REST repeated calls are deterministic and one-note bounded', async (t) => {
  const canisterCalls = [];
  const canister = express();
  canister.get('/api/v1/notes/:path', (req, res) => {
    canisterCalls.push(req.originalUrl);
  ...
  for (let index = 0; index < 5; index += 1) {
    const res = await fetch(`${gateway.url}/api/v1/note-outline?path=repeat.md`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
test('data-integrity: NoteOutline REST rejects unsafe paths before upstream fetch', async (t) => {
  const canisterCalls = [];
  const canister = express();
  canister.get(/.*/, (req, res) => {
    canisterCalls.push(req.originalUrl);
  ...
  for (const unsafePath of ['../secret.md', '/Users/owner/secret.md', 'C:\\Users\\owner\\secret.md']) {
    const res = await fetch(`${gateway.url}/api/v1/note-outline?path=${encodeURIComponent(unsafePath)}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const body = await res.json();
    const serialized = JSON.stringify(body);
...
test('performance: NoteOutline REST does not call bridge search, index, memory, or providers', async (t) => {
  const bridgeCalls = [];
  const bridge = express();
  ...
  const res = await fetch(`${gateway.url}/api/v1/note-outline?path=a.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
test('security: NoteOutline REST sanitizes missing, unauthorized, and upstream failures', async (t) => {
  const canister = express();
  ...
  const missing = await fetch(`${gateway.url}/api/v1/note-outline?path=missing.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const forbidden = await fetch(`${gateway.url}/api/v1/note-outline?path=private.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const broken = await fetch(`${gateway.url}/api/v1/note-outline?path=broken.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
