 ...
 *
 * Phase 1E adds the local CLI command, and later phases may add body-free
 * self-hosted MCP only. It must not add hosted MCP, Hub, search, persistence,
 * Scooling runtime behavior, section bodies, snippets, summaries, PageIndex,
 * OCR, LLM calls, or provider routing.
 */
...
const specPath = path.join(repoRoot, 'docs', 'SECTION-SOURCE-CLI-IMPLEMENTATION-SPEC.md');

function readRepoFile(relativePath) {
  return fs.readFileSync(path.join(repoRoot, relativePath), 'utf8');
}

describe('SectionSource Phase 1E local CLI implementation', () => {
  it('unit: spec covers the required local CLI implementation areas', () => {
    ...
    assert.match(spec, /knowtation get-section-source <path> --json/);
    assert.match(spec, /This phase adds the local CLI command only/);
  });

  it('integration: current SectionSource local integration remains the expected CLI target', () => {
  ...
  it('end-to-end: local CLI, self-hosted MCP, and hosted MCP are registered without Hub wiring', () => {
    ...
    assert.equal(hubServer.includes('get-section-source'), false);
    assert.equal(hubServer.includes('get_section_source'), false);
    assert.equal(hubServer.includes('section_source/v0'), false);
  });

  it('stress: CLI spec checks remain bounded to contract files', () => {
    ...
    assert.ok(elapsedMs < 200, `expected bounded CLI spec check under 200ms, got ${elapsedMs}ms`);
  });

  it('data-integrity: local CLI adds no writes, sidecars, indexes, vectors, or summaries', () => {
  ...
  it('performance: spec requires one-note reads and no external providers', () => {
    ...
    assert.match(spec, /The command reads one note only/);
    assert.match(spec, /The command does not scan the whole vault/);
    assert.match(spec, /The command does not call external providers/);
  });

  it('security: spec blocks body, snippet, hosted, search, persistence, and Scooling exposure', () => {
    const spec = fs.readFileSync(specPath, 'utf8');
