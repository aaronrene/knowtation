/**
 * Gateway REST SectionSource tests.
 *
 * Phase 1N exposes body-free SectionSource v0 through Hub gateway REST only.
 * It must preserve auth, active vault, effective canister user, one-note reads,
 * path safety, sanitized errors, and the no-body/no-snippet output boundary.
 */
...
const SECRET = 'gateway-section-source-rest-test-secret-32';
const GATEWAY_AUTH_SECRET = 'gateway-section-source-rest-gw-secret';
...
async function startGateway(canisterUrl, bridgeUrl) {
  ...
  const { app: gwApp } = await import(`${gwEntry}?gwsection=${Date.now()}-${Math.random()}`);
  return startServer(gwApp);
}

test('unit: GET /api/v1/section-source is auth-gated before upstream fetch', async (t) => {
  const canisterCalls = [];
  ...
  const res = await fetch(`${gateway.url}/api/v1/section-source?path=inbox/a.md`);
...
test('integration: SectionSource REST uses active vault and effective canister user headers', async (t) => {
  const bridgeCalls = [];
  ...
  canister.get('/api/v1/notes/:path', (req, res) => {
    ...
    res.json({
      ...
      frontmatter: '{"title":"REST Section","api_key":"must-not-leak"}',
      body: '# Intro\n\nBody must not leak.\n\n## Next',
    });
  });
  const canisterSrv = await startServer(canister);
  ...
  const res = await fetch(`${gateway.url}/api/v1/section-source?path=${encodeURIComponent('inbox/hello world.md')}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      'X-Vault-Id': 'vault-rest',
    },
  });
  const body = await res.json();
  ...
  assert.equal(bridgeCalls[0].vault, 'vault-rest');
  ...
  assert.equal(canisterCalls[0].vault, 'vault-rest');
  assert.equal(canisterCalls[0].gatewayAuth, GATEWAY_AUTH_SECRET);
  assert.equal(body.schema, 'knowtation.section_source/v0');
  assert.equal(body.path, 'inbox/hello world.md');
});

test('end-to-end: SectionSource REST returns the body-free allowlist only', async (t) => {
  const canister = express();
  ...
  const token = signTestJwt({ sub: 'google:actor', role: 'viewer' });
  const res = await fetch(`${gateway.url}/api/v1/section-source?path=safe.md`, {
...
test('stress: SectionSource REST repeated calls are deterministic and one-note bounded', async (t) => {
  const canisterCalls = [];
  const canister = express();
  canister.get('/api/v1/notes/:path', (req, res) => {
    canisterCalls.push(req.originalUrl);
  ...
  for (let index = 0; index < 5; index += 1) {
    const res = await fetch(`${gateway.url}/api/v1/section-source?path=repeat.md`, {
      headers: { Authorization: `Bearer ${token}` },
    });
...
test('data-integrity: SectionSource REST rejects unsafe paths before upstream fetch', async (t) => {
  const canisterCalls = [];
  const canister = express();
  canister.get(/.*/, (req, res) => {
    canisterCalls.push(req.originalUrl);
  ...
  for (const unsafePath of ['../secret.md', '/Users/owner/secret.md', 'C:\\Users\\owner\\secret.md']) {
    const res = await fetch(`${gateway.url}/api/v1/section-source?path=${encodeURIComponent(unsafePath)}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const body = await res.json();
    const serialized = JSON.stringify(body);
...
test('performance: SectionSource REST does not call bridge search, index, memory, or providers', async (t) => {
  const bridgeCalls = [];
  const bridge = express();
  ...
  const res = await fetch(`${gateway.url}/api/v1/section-source?path=a.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
test('security: SectionSource REST sanitizes missing, unauthorized, and upstream failures', async (t) => {
  const canister = express();
  ...
  const missing = await fetch(`${gateway.url}/api/v1/section-source?path=missing.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const forbidden = await fetch(`${gateway.url}/api/v1/section-source?path=private.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
  const broken = await fetch(`${gateway.url}/api/v1/section-source?path=broken.md`, {
    headers: { Authorization: `Bearer ${token}` },
  });
