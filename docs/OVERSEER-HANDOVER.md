...
**SEC-KN-2 DONE (2026-07-26):** Client `evaluation_status` / `evaluated_by` / `evaluated_at` are
stripped on every proposal-create augment; only server policy pending, E1 (server audit), or the
evaluate endpoint may set them. Seven-tier tests green; build verification **pass**. Code on
`feat/sec-kn-2-server-only-evaluation`. **Not merged to main.**

```text
SEC-KN-3 — agent tokens cannot inherit admin (Pass 2 P6).
...
- hub/gateway/server.mjs (resolveHostedActorRole ~2976-3001)
- hub/gateway/access-token-authz.mjs (~4-7, 45-52)
- lib/hub-proposal-personal-self-apply.mjs (roleEligibleForPersonalSelfApply ~75-78)
- ~/scooling/docs/PRE-BUILD-SECURITY-AUDIT-FINDINGS-PASS2.md (finding P6)

Do:
1) When payload.type === 'mcp_access', cap role by the token's own scopes; never apply the
   admin-allowlist override in resolveHostedActorRole.
2) Add a human-actor test to roleEligibleForPersonalSelfApply so agent tokens are never
   self-apply eligible.
3) Add a security-tier regression test that fails against the pre-fix allowlist inheritance.
...
- [ ] Freeze review — N/A if implementing against Pass 2 P6 citation as the frozen requirement;
...
- [ ] **SEC-KN-3** — P6 `mcp_access` role cap + no self-apply for agent tokens (**Auto**) — **NEXT**
