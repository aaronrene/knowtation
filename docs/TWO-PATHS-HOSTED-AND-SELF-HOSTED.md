...
You do **not** need two separate products or two codebases. The **same** repo and UI support both. The only difference is **which URL they open** and **which backend** serves the API (our gateway + canister vs your Node Hub).
...
**Goal:** Someone can clone, set two things, and open the Hub in a few minutes.
...
They’ll see the same Hub UI. Without OAuth they can still use the API with a token; for **Continue with Google/GitHub** they add `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` (and the redirect URI in Google/GitHub) as in [hub/README.md](../hub/README.md).

**Optional:** A **Quick start (self-hosted)** page in the repo or on the site that is literally these steps + links to hub/README and GETTING-STARTED for config details.

---

## Hosted path: GitHub backup (Connect GitHub, Back up now)

**What we offer:** Users on the **hosted** Hub can connect **their** GitHub account so **we** push their vault (notes) to **their** repo. They don’t run git themselves; they click **Connect GitHub** and **Back up now** in Settings.

**What “git” does under the hood (we run it, not the user):**

- **Connect GitHub:** OAuth to GitHub; we store a token (scope `repo`) and optionally the repo they want (e.g. `owner/repo-name`).
- **Back up now:** We fetch their vault from the canister (GET `/api/v1/export`), then create a commit with all note files and push to their repo via the **GitHub API** (we don’t run a local `git` binary; we use GitHub’s “create blob / tree / commit / push” API). So the **commands** are effectively: create blobs, build tree, create commit, update ref (push). No raw `git add` / `git commit` in a terminal — it’s all through the API.
...
- **If they ever exceed free tier:** That’s a GitHub account limit (e.g. many repos or large repos). For a single “vault backup” repo with markdown, we don’t expect users to hit it. You can document: “Vault is markdown; well under GitHub’s free limits.”

**Summary:** One product, two paths (hosted + self-hosted). Hosted users can hook up GitHub so we commit and push their data to their repo; we use GitHub’s API, not raw git commands; storage is text-only and fits easily in GitHub’s free tier.
