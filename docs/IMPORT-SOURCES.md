...
| 📊 `generic-csv`     | Path to a **single** `.csv` file (UTF-8; optional BOM) | **Tabular** import: first row = headers, each following row = one note. Body lists each column as a bullet. Frontmatter: `source: csv-import`, **`title`** (includes the **filename** and, if a column named **`title`**, **`name`**, **`subject`**, **`summary`**, or **`label`** has a value, that value; otherwise **`filename (row N)`** for that data row), `source_id` (from `id` / `uuid` / `key` column if present, else content hash), `csv_file`, `row_index`, `date`. The note body H1 matches **`title`**. Max **10,000** data rows, **50 MB** file, **32,000** chars per cell (truncated). **Google Sheets (download):** *File → Download → Comma-separated values (.csv)* then import. |
...
| 📗 `excel-xlsx`      | Path to a **single** `.xlsx` file (Office Open XML) | **Tabular** import from the **first worksheet** only, same model as `generic-csv` (header row, one note per data row, same **`title`** rules). `source: xlsx-import`, `xlsx_file`, `row_index`, `date`. **Legacy** `.xls` is not supported. Max **50 MB** file, **10,000** rows, **32,000** characters per cell (truncated). |
