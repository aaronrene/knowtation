...
**Next hosted tool (sixteenth once registered):** **`capture`** (ACL **editor** — inventory still **No**). Tracked on branch **`feature/hosted-mcp-capture`**; confirm bridge/canister upstream before `registerTool`. **`transcribe`** remains the next candidate after **`capture`** is designed.
...
| Documented operator smoke for **each** of the fifteen tools after deploy | **`backlinks`**, **`extract_tasks`**, **`cluster`**, and **`tag_suggest`** smoke recorded (2026-04). See § *Production verification* subsections. Run the rest by following [§ How to test hosted MCP](#how-to-test-hosted-mcp). |
...
| **Implementing** the next hosted tool | Branch **`feature/hosted-mcp-capture`**: **`capture`** (ACL editor, inventory **No**) — confirm upstream on bridge or canister, then `registerTool` + golden tests per this doc. After **`capture`**, **`transcribe`** is the next ACL-listed candidate without a hosted handler. |

Pick **one** tool per PR. **`tag_suggest`** is registered and smoke-verified on hosted MCP. **`capture`** is the active next target on **`feature/hosted-mcp-capture`**; **`transcribe`** follows once **`capture`** is designed (see inventory table).
