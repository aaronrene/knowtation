...
A **fifteenth** tool, **`tag_suggest`** (viewer), reads the source note on the canister (or accepts raw **`body`**), runs semantic **`POST /api/v1/search`** on the bridge with **`limit` 15**, and aggregates **`tags`** from results (with **`GET …/notes/:path`** fallback per neighbor) — see the inventory row and § *Production verification: fifteenth tool `tag_suggest`*.

What **is** still unwired: the **remaining names** in [`hub/gateway/mcp-tool-acl.mjs`](../hub/gateway/mcp-tool-acl.mjs) (`capture`, `transcribe`, …) that are **not** yet registered in the hosted server. Those are **future** tools, not partial work from the safeguards session.
...
**Next hosted tool (sixteenth, not started here):** pick the next ACL-listed name that still shows **No** in the inventory table (e.g. **`capture`** / **`transcribe`** — confirm upstream before implementing).
...
2. Optional: open the MCP panel / tool list and confirm you see **fifteen** tools if your Hub role is **admin** (fewer if viewer or editor).
...
|------|----------------------|-------------------------|
| 0 | Server lists tools | `Using only the knowtation-hosted MCP server: list the tool names available to you and confirm there are fifteen tools if I am an admin (including relate, backlinks, extract_tasks, cluster, and tag_suggest).` |
...
- **`tag_suggest`:** Canister **`GET …/notes/:path`** when **`path`** is set, or caller **`body`** only (12k slice). Bridge **`POST /api/v1/search`** semantic (**`limit` 15**, **`snippetChars` 200**) — same auth as **`relate`** (`resolveHostedBridgeContext` on the bridge). Tags from each search row’s **`tags`** field when present; otherwise **`GET …/notes/:path`** for that neighbor. **EC2 smoke:** follow step **4f** after deploy; see § *Production verification: fifteenth tool `tag_suggest`* and the inventory row.
...
| `tag_suggest` | viewer | Yes | Canister `GET …/notes/:path` when **`path`** is set (same headers as **`get_note`**); optional **`body`** only (12k slice, same cap as `lib/tag-suggest.mjs`). Neighbors: bridge **`POST /api/v1/search`** semantic with **`limit: 15`**, **`snippetChars: 200`** — JWT `Authorization` + `X-Vault-Id` + **`resolveHostedBridgeContext`** (same hosted model as **`relate`** / `hub/bridge/server.mjs` `POST /api/v1/search`; bridge embeds **`query`** with **`voyageInputType: "query"`** — intentional gap vs local **document** embedding, documented like **`relate`**). Result rows include **`tags`** when the vector store exposes them; empty **`tags`** → **`GET …/notes/:path`** per neighbor for `tagsFromFm` / frontmatter. Returns **`suggested_tags`** (up to 12, slug frequency) and **`existing_tags`**. **EC2 production smoke:** pending after gateway deploy. |
...
| **Fifteen** tools on hosted MCP | Implemented in `mcp-hosted-server.mjs`: bridge/canister `upstreamFetch` for JSON APIs; **`import`** uses multipart `fetch` to the bridge; **`vault_sync`** POSTs JSON to the bridge; **`export`** GETs canister `/api/v1/export` with a byte cap; **`relate`** + bridge semantic search; **`backlinks`** + canister list/get + `lib/wikilink.mjs`; **`extract_tasks`** + canister list + `lib/extract-tasks.mjs`; **`cluster`** + canister list/get + bridge **`POST /api/v1/embed`** + `lib/kmeans.mjs`; **`tag_suggest`** + canister read + bridge **`POST /api/v1/search`** + optional per-neighbor canister reads for tags (see inventory table). |
...
| Item | Meaning |
|------|---------|
| Hosted `registerTool` for `capture`, `transcribe` | No Cursor-visible tool until implemented; ACL alone does nothing. |
...
| Documented operator smoke for **each** of the fifteen tools after deploy | **`backlinks`**, **`extract_tasks`**, and **`cluster`** smoke recorded (2026-04); **`tag_suggest`** smoke pending post-deploy. See § *Production verification* subsections. Run the rest by following [§ How to test hosted MCP](#how-to-test-hosted-mcp). |
...
**Status:** Implemented in repo in [`hub/gateway/mcp-hosted-server.mjs`](../hub/gateway/mcp-hosted-server.mjs); **EC2 production smoke** should be run after gateway deploy.
...
| Work | Where |
|------|--------|
| **Manual smoke** of the fifteen tools on your EC2 MCP URL | After deploy: follow [§ How to test hosted MCP](#how-to-test-hosted-mcp), including step **4e** for `cluster`, step **4f** for `tag_suggest`, step **6b** for `import`, step **6c** for `export` if you are admin, and step **8** for `vault_sync` when GitHub is connected. |
...
Pick **one** tool per PR. **`tag_suggest`** is registered on hosted MCP; the next ACL-listed candidates without hosted handlers include **`capture`** and **`transcribe`** (confirm upstream before implementing — see inventory table).
