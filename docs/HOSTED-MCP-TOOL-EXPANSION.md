...
**Status:** **Merged to `main`** (PR **#174**, April 2026): five **`registerPrompt`** handlers in [`hub/gateway/mcp-hosted-server.mjs`](../hub/gateway/mcp-hosted-server.mjs); ACL in [`hub/gateway/mcp-tool-acl.mjs`](../hub/gateway/mcp-tool-acl.mjs); tests in [`test/mcp-hosted-prompts.test.mjs`](../test/mcp-hosted-prompts.test.mjs). **`npm run verify:hosted-mcp-checklist`** includes `prompts/list` coverage. **Next:** Track B2 on branch **`feat/hosted-mcp-prompts-b2`** (see [`NEXT-SESSION-HOSTED-HUB-MCP.md`](./NEXT-SESSION-HOSTED-HUB-MCP.md)).
