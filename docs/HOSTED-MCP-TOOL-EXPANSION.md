...
What **is** still unwired: the **remaining names** in [`hub/gateway/mcp-tool-acl.mjs`](../hub/gateway/mcp-tool-acl.mjs) (for example **`transcribe`**) that are **not** yet registered in the hosted server. Those are **future** tools, not partial work from the safeguards session.
...
2. Optional: open the MCP panel / tool list and confirm you see **sixteen** tools if your Hub role is **admin** (fewer if viewer or editor).
...
|------|----------------------|-------------------------|
| 0 | Server lists tools | `Using only the knowtation-hosted MCP server: list the tool names available to you and confirm there are sixteen tools if I am an admin (including relate, backlinks, extract_tasks, cluster, tag_suggest, and capture).` |
...
| **Sixteen** tools on hosted MCP | Implemented in `mcp-hosted-server.mjs`: bridge/canister `upstreamFetch` for JSON APIs; **`import`** uses multipart `fetch` to the bridge; **`vault_sync`** POSTs JSON to the bridge; **`export`** GETs canister `/api/v1/export` with a byte cap; **`relate`** + bridge semantic search; **`backlinks`** + canister list/get + `lib/wikilink.mjs`; **`extract_tasks`** + canister list + `lib/extract-tasks.mjs`; **`cluster`** + canister list/get + bridge **`POST /api/v1/embed`** + `lib/kmeans.mjs`; **`tag_suggest`** + canister read + bridge **`POST /api/v1/search`** + optional per-neighbor canister reads for tags; **`capture`** + canister **`POST …/notes`** via **`buildCaptureInboxWritePayload`** (see inventory table). |
...
| Item | Meaning |
|------|---------|
| Hosted `registerTool` for `transcribe` | No Cursor-visible tool until implemented; ACL alone does nothing. |
...
| Documented operator smoke for **each** of the sixteen tools after deploy | **`backlinks`**, **`extract_tasks`**, **`cluster`**, **`tag_suggest`**, and **`capture`** (post-merge EC2) smoke tracked in § *Production verification* subsections. Run the rest by following [§ How to test hosted MCP](#how-to-test-hosted-mcp). |
