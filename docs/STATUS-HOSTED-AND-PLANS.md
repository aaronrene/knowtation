...
| **Bulk delete by project slug / rename project** | Node Hub `POST /notes/delete-by-project`, `POST /notes/rename-project` | **Not on canister**; Hub UI hidden for canister vaults — **next:** branch **`feature/hosted-delete-rename-parity`** ([HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md), [PARITY-PLAN.md](./PARITY-PLAN.md)) |
...
| **Hosted multi-vault (canister) — Phase 15.1** | **Repo:** merged (**#46–#48**) + **workspace / vault-access / scope** ([HOSTED-WORKSPACE-ACCESS.md](./HOSTED-WORKSPACE-ACCESS.md)). **Production:** redeploy gateway + bridge + §5.1 if needed. **Next gap vs self-hosted:** **Hub Import** (501) — [PARITY-PLAN.md](./PARITY-PLAN.md). |
