...
| **SEC-KN-1** | **Auto** | **TODO — NEXT** | **P1 fail-closed.** Change the empty-secret branch in `gatewayAuthorized` to **deny**. Add a startup/health assertion so an unset secret is loud, not silent. Seven-tier tests including a security tier that asserts denial on empty secret and on a forged `X-User-Id` without valid `X-Gateway-Auth`. |
