...
| **FLOW-WRITE-LIVE-GATEWAY-PROXY** | **Auto** | **DONE + landed 2026-07-29** — BV round 1 = `pass`; seven-tier **12/12**; Muse/`main` → muse-mirror | **Gateway→bridge Flow authoring proxies.** `POST /api/v1/flows`, `…/:id/proposals`, `…/import` before canister catch-all; bridge `registerBridgeFlowRoutes` + `createFlowProposalOnCanister`. Capture/run/Delegation unchanged. No `FLOW_AUTHORING_WRITES` flip this session. BV: `docs/reviews/2026-07-29-flow-write-live-gateway-proxy-bv-round1-pass.md`. Product NEXT = Operator §FWL.9 retry (confirm bridge `FLOW_AUTHORING_WRITES`). |
