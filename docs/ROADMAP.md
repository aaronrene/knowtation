...
| **Gate** | Scooling's `FINISH-COMPLETE-APPLY-KN-b` is **NO-GO** until **SEC-KN-1 verified** and **SEC-KN-2 shipped** |

## Build queue

| Phase | Model | Status | Deliverable |
| --- | --- | --- | --- |
| **SEC-KN-0** | **Operator** | **TODO — first** | **Verify the canister gateway auth secret is actually set.** `main.mo:930-939` returns `true` when `gateway_auth_secret` is empty; `main.mo:153-158,1017` derives identity from a raw `X-User-Id` header defaulting to `"default"`. If the secret is unset, **every** partition, discard, and evaluation guarantee is bypassable by direct canister HTTP. Nothing else on this board is meaningful until this is answered. |
| **SEC-KN-1** | **Auto** | **TODO** | **P1 fail-closed.** Change the empty-secret branch in `gatewayAuthorized` to **deny**. Add a startup/health assertion so an unset secret is loud, not silent. Seven-tier tests including a security tier that asserts denial on empty secret and on a forged `X-User-Id` without valid `X-Gateway-Auth`. |
