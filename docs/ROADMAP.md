...
| **CAPTURE-STORE-BLOB-PERSIST (9-kn-c)** | **Auto** | **IN PROGRESS 2026-07-31** — fix built + seven-tier **11/11** (pre-fix routes fail **7/11**, proving the regression is caught); pending land via muse-mirror PR + live capture re-run | Root cause of the 9-apply live failure (`409 FLOW_CANDIDATE_NOT_PROMOTABLE` on `prop-1785500300353491755`): Wave 2 bridge capture routes (observe / candidates / propose / dismiss in `hub/bridge/flow-capture-routes.mjs`) never hydrated/persisted the flow store through Netlify Blobs, so candidates lived only on the warm lambda's ephemeral disk and were gone by approve time. Fix wraps the three mutating routes in `withExternalProtocolBlobSync` and hydrates before the candidates read — same pattern the CHA apply/list routes already use. Regression suite `test/capture-store-blob-persist.test.mjs` simulates separate lambda instances as separate DATA_DIRs sharing one blob store (unit/integration/e2e/stress/data-integrity/performance/security). After land: fresh observe → propose → operator approve → expect `capture_index_applied: true` + Flow listed. |
