...
| **Gate** | Scooling's `FINISH-COMPLETE-APPLY-KN-b` is **NO-GO** until **SEC-KN-1** canister upgrade (Tier 3) and remaining SEC queue items that the Scooling freeze carries (P3/P4/P6). **SEC-KN-2 shipped** (code on `feat/sec-kn-2-server-only-evaluation`). |
