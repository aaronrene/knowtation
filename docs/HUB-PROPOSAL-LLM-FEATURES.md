...
- The **gateway** may run a **bounded inline** job after a **successful** **`POST /api/v1/proposals`**: when hints are enabled (`KNOWTATION_HUB_PROPOSAL_REVIEW_HINTS` / admin prefs), it calls `maybeScheduleHostedProposalReviewHints` (`hub/gateway/proposal-review-hints-async.mjs`). Netlify freezes after the handler returns, so the job must finish (or hit its deadline) **before** the create response is sent — it is **not** true background.
- **Inline budget:** default **`HOSTED_PROPOSAL_REVIEW_HINTS_INLINE_BUDGET_MS = 2000`**. Callers cannot raise above that cap. This leaves headroom for Scooling’s 15s propose+approve abort (`HOSTED_REVIEW_WRITE_BACK_DEFAULT_TIMEOUT_MS`). (An earlier 18s default caused production one-click Approve timeouts on 2026-07-23.)
- **Scooling personal self-apply skip:** creates with intent `scooling.review_tray.approve` **skip** inline hints entirely — learner one-click approve follows create immediately; Hub reviewer hints are not on that path (SD-18).
- Hints appear in the Hub drawer when present (refresh if needed). Empty hints after timeout/failure are normal — check gateway logs for `review hints failed`.
