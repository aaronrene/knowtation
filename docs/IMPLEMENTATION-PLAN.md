...
| **Next** | **AIR Improvement D — Built-in attestation endpoint** (`feature/air-built-in-endpoint`): Netlify Function `POST /api/v1/attest` (HMAC-signed records in Netlify Blobs) + `GET /api/v1/attest/:id` verify; gateway auto-sets `KNOWTATION_AIR_ENDPOINT` when `ATTESTATION_SECRET` present. Planning pass first — see [AIR-IMPROVEMENTS-PLAN.md](./AIR-IMPROVEMENTS-PLAN.md) §D. | Branch created; implement after plan review. |
...
| **Planned** | **Phase 12B (Blockchain remainder):** (1) Wallet/transaction history CSV import (`wallet-csv` source type — column mapping, dedup by `tx_hash`, generic + named parsers); (2) AIR on-chain backend — ICP attestation canister as tamper-evident backing store (requires AIR D first). See [PHASE12B-BLOCKCHAIN-REMAINDER.md](./PHASE12B-BLOCKCHAIN-REMAINDER.md). | Branch: `feature/phase12b-blockchain-remainder`. Start after 12A merges. |
| **Done/In Progress** | **AIR Improvements (A–E):** ✅ (A) Store `air_id` in frontmatter — **done** (PR #96). ✅ (B) Gateway AIR for hosted writes — **done** (PR #96). ✅ (C) `air.required` hard-fail — **done** (PR #96). 🔲 (D) Built-in Netlify endpoint — **next** (`feature/air-built-in-endpoint`). 🔲 (E) ICP blockchain anchor — future (`feature/air-icp-attestation`). See [AIR-IMPROVEMENTS-PLAN.md](./AIR-IMPROVEMENTS-PLAN.md). | A+B+C merged to `main`. D = planning pass → implement. E = after D. |
