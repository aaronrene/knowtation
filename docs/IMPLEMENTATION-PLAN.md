...
| **Then** | **Phase 15.1 hosted multi-vault parity:** Canister `(userId, vault_id)` + settings/vault list + bridge index/search per vault — [MULTI-VAULT-AND-SCOPED-ACCESS.md](./MULTI-VAULT-AND-SCOPED-ACCESS.md). **Defined in plan:** § **Phase 15** → **Phase 15.1 — Hosted multi-vault parity** below. | Primary product gap vs self-hosted until canister redeploy + verify ([STATUS-HOSTED-AND-PLANS.md](./STATUS-HOSTED-AND-PLANS.md) §2). |
