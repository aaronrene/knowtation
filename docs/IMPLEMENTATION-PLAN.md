...
| **Next (hosted)** | **Phase 2 ops:** Bridge live, `BRIDGE_URL`, pre-roll, smoke tests on knowtation.store. | Before multi-vault on hosted. |
| **Then** | **Phase 15.1 hosted multi-vault:** Canister storage by `vault_id` + settings/vault list source + backup/index per vault (see MULTI-VAULT checklist). Greenfield: minimal migration. | After Phase 2 ops. |
| **Later** | **Phase 16:** Hosted credits. **Phase 12:** Blockchain when needed. **MCP D2/D3, F2–F5** per backlog. | After hosted MVP slice. |
...
| — | **Hosted / ICP (canister)** | Deployed | Canister, 4Everland (knowtation.store), Netlify gateway (knowtation-gateway.netlify.app), DNS. **Pre-roll not confirmed** (see [STATUS-VERIFICATION.md](./STATUS-VERIFICATION.md)). Parity (gateway stubs) merged. **Remaining:** canister redeploy for Option B fields; Netlify + 4Everland rebuild; **bridge deploy and wire (required)**. **Phase 15 (multi-vault):** **self-hosted done**; **hosted** still needs canister storage by `(uid, vault_id)` (see [MULTI-VAULT-AND-SCOPED-ACCESS.md](./MULTI-VAULT-AND-SCOPED-ACCESS.md) § Hosted). Do not rely on hosted multi-vault **content isolation** until that ships; finish **Phase 2 bridge + env** before large hosted multi-vault effort. See [PARITY-PLAN.md](./PARITY-PLAN.md) and [STATUS-HOSTED-AND-PLANS.md](./STATUS-HOSTED-AND-PLANS.md). |
