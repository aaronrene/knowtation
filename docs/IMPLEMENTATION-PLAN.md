...
| **Then (hosted)** | **Hub Import** on hosted (replace gateway **501**; multipart import to canister path). | [PARITY-PLAN.md](./PARITY-PLAN.md). |
...
3. **Multi-vault on hosted:** Phase 15.1 **partition + UI bootstrap** merged (**#46–#48**); **team vault access + scope** in repo ([HOSTED-WORKSPACE-ACCESS.md](./HOSTED-WORKSPACE-ACCESS.md)). **Remaining gap vs self-hosted:** **Hub Import**; **ops:** redeploy gateway + bridge + §5.1 when needed.
