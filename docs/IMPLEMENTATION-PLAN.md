...
| **Planned** | **AIR Improvements (A–E):** (A) Store `air_id` in note frontmatter after attestation; (B) Wire hosted gateway writes to AIR; (C) Hard-fail config flag (`air.required`); (D) Built-in Netlify attestation endpoint with HMAC + Blobs storage; (E) ICP blockchain anchor for tamper-evident audit trail. See [AIR-IMPROVEMENTS-PLAN.md](./AIR-IMPROVEMENTS-PLAN.md). | Branch: `feature/air-improvements` (A+B+C), `feature/air-built-in-endpoint` (D), `feature/air-icp-attestation` (E). |
