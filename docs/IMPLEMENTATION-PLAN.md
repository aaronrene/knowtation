...
| **Next** | **Hosted parity — metadata bulk delete/rename:** Same two POST routes (or equivalent) against canister storage **or** authenticated bridge/gateway batch; then show Hub UI for canister (`settings-bulk-metadata-self-only`). Branch: **`feature/hosted-delete-rename-parity`**. | [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md), [PARITY-PLAN.md](./PARITY-PLAN.md) hosted table. |
...
3. **Hosted metadata bulk (delete-by-project / rename-project):** Self-hosted shipped (**PR #63**). **Next:** canister-native Motoko and/or gateway–bridge implementation; branch **`feature/hosted-delete-rename-parity`**. **Spec:** [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md).
...
5. **Multi-vault on hosted:** Phase 15.1 **partition + UI bootstrap** merged (**#46–#48**); **team vault access + scope** in repo ([HOSTED-WORKSPACE-ACCESS.md](./HOSTED-WORKSPACE-ACCESS.md)). **Ops:** redeploy gateway + bridge + §5.1 when needed.
6. **Memory:** Compare **file memory (Phase 8)** vs **Mem0 import/API** on the same vault; pick one primary story for docs and MCP until requirements firm up.
...
1. **Indexing + search verification** — Same as **Recommended path forward** §1; confirm CLI and Hub-hosted (bridge) paths. Remaining parity items: [PARITY-PLAN.md](./PARITY-PLAN.md) (e.g. **hosted metadata bulk** delete/rename — import on hosted is live when **`BRIDGE_URL`** is set).
...
| 12 | **Hub bulk delete / rename + folder UX** | Done (self-hosted) / **Next (hosted)** | **PR #63:** Path-prefix delete unchanged on canister; **delete-by-project** + **rename-project** on **Node Hub** only; Settings presets + New note folder select; `GET /vault/folders` (full list self-hosted, stub `inbox` on gateway). **Hosted:** implement parity on canister or bridge — branch **`feature/hosted-delete-rename-parity`**; [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md). |
