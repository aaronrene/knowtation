...
| **Done (hosted)** | **Metadata bulk delete/rename:** Gateway handlers orchestrate the canister (`hub/gateway/metadata-bulk-canister.mjs`); shared `effectiveProjectSlug` in `lib/vault.mjs`; Hub UI unhidden for canister vaults. **Ops:** redeploy gateway. | [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md), [PARITY-PLAN.md](./PARITY-PLAN.md). |
...
3. **Hosted metadata bulk (delete-by-project / rename-project):** Self-hosted **PR #63**; hosted **gateway** implementation in **`hub/gateway/metadata-bulk-canister.mjs`**. **Spec:** [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md). **Ops:** redeploy gateway; optional later Motoko-native bulk if needed.
...
1. **Indexing + search verification** — Same as **Recommended path forward** §1; confirm CLI and Hub-hosted (bridge) paths. Parity: [PARITY-PLAN.md](./PARITY-PLAN.md); **hosted metadata bulk** is on the gateway (redeploy gateway); **import** on hosted is live when **`BRIDGE_URL`** is set.
...
| 12 | **Hub bulk delete / rename + folder UX** | Done (self-hosted + hosted gateway) | **PR #63** (Node); **gateway** [`metadata-bulk-canister.mjs`](hub/gateway/metadata-bulk-canister.mjs) (hosted project-slug bulk); path-prefix delete still canister; `GET /vault/folders` stub `inbox` on gateway. [HUB-METADATA-BULK-OPS.md](./HUB-METADATA-BULK-OPS.md). |
