...
4. **Issues #1 and #2 (MCP supercharge)** — Capture requirements from [GitHub Issue #1](https://github.com/aaronrene/knowtation/issues/1) and [Issue #2](https://github.com/aaronrene/knowtation/issues/2) into [BACKLOG-MCP-SUPERCHARGE.md](./BACKLOG-MCP-SUPERCHARGE.md). Those issues (Phase A–E: resource prompts, streaming transport, subscriptions, sampling, infinite machine brain) are not accessible from automation (404); paste issue content into the backlog doc so we can break into tasks and decide what to implement now vs after Phase 2. See BACKLOG-MCP-SUPERCHARGE for timing and current state.
...
| — | **Hosted / ICP (canister)** | Deployed | Canister, 4Everland (knowtation.store), Netlify gateway (knowtation-gateway.netlify.app), DNS. **Pre-roll not confirmed** (see [STATUS-VERIFICATION.md](./STATUS-VERIFICATION.md)). Parity (gateway stubs) merged. **Remaining:** canister redeploy for Option B fields; Netlify + 4Everland rebuild; **bridge deploy and wire (required)**. Do not start Phase 15 (multi-vault) until Phase 2 including bridge is complete. See [PARITY-PLAN.md](./PARITY-PLAN.md) and [STATUS-HOSTED-AND-PLANS.md](./STATUS-HOSTED-AND-PLANS.md). |
