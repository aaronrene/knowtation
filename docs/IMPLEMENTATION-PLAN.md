...
**Status for next session:** **→ [NEXT-SESSION.md](./NEXT-SESSION.md)** (copy-paste prompt + ordered steps). Also [STATUS-HOSTED-AND-PLANS.md](./STATUS-HOSTED-AND-PLANS.md), [PARITY-PLAN.md](./PARITY-PLAN.md). **Sequencing:** MCP / Issue #1 supercharge **on `main`**. **First:** **`npm test` green** (fix fixtures / config tests). **Then:** hosted Phase 2 — bridge, `BRIDGE_URL`, pre-roll, smoke. **Then:** **Phase 15.1** hosted multi-vault — [MULTI-VAULT-AND-SCOPED-ACCESS.md](./MULTI-VAULT-AND-SCOPED-ACCESS.md). **Then** MCP **D2/D3**, **F2–F5** — [BACKLOG-MCP-SUPERCHARGE.md](./BACKLOG-MCP-SUPERCHARGE.md). **Issue #2** deferred.
