...
| **Later** | **Phase 16:** Hosted **Stripe** + **indexing token** metering (bridge token counts → gateway ledger → Hub UI) — [HOSTED-CREDITS-DESIGN.md](./HOSTED-CREDITS-DESIGN.md) §4. **Phase 12:** Blockchain when needed. **MCP D2/D3, F2–F5** per backlog. | After pricing doc + bridge instrumentation. |
...
6. **Phase 12 (blockchain / wallets) — optional, not billing-dependent:** Knowtation remains a **knowledge** product; you do not need in-app **transaction signing** for core value. A **reasonable** scope when needed: **read-only** on-chain context in notes (SPEC frontmatter, links), **MCP or external tools** that fetch **public** chain data for agents, and **plugins** documented in [BLOCKCHAIN-AND-AGENT-PAYMENTS.md](./BLOCKCHAIN-AND-AGENT-PAYMENTS.md). Treat **custodial wallets inside the Hub** as out-of-scope unless a partner requires it.
...
5. **Hub UI** — Usage bar: **indexing tokens used / included**, **pack balance**, **Buy pack** / Portal; **`cost_breakdown`** may stay for writes until token-only UX ships.
