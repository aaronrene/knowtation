...
12. ~~**MCP D2/D3, F2–F5:**~~ ✅ Done (PR #101 merged). Hub MCP gateway (`/mcp` endpoint, session pool, role ACL), OAuth 2.1 (`KnowtationOAuthProvider`), sampling tools (enrich, rerank, prefill, index-enrich). **AWS gateway deployed** on EC2 `18.221.120.124` (PM2 + Nginx, systemd auto-start). Tooling history: **`mcp/tools/phase-c.mjs`**, **`hub/gateway/mcp-*.mjs`**, [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §2.
13. ~~**Phase 18 (Media URL in notes + MCP image/video resources):**~~ ✅ Done (`feature/phase18-image-resources`, hosted parity on `main`). Knowtation does NOT host media. (A) MCP Image Resources — `note-image` ResourceTemplate with SSRF-protected fetcher (`image-fetch.mjs`), base64 blob for vision models; (B) MCP Video Resources — `note-video` ResourceTemplate, URL as text with typed `video/*` mimeType; (C) Hub `<video controls>` inline renderer + DOMPurify hardening; (D) GitHub-commit image upload — `lib/github-commit-image.mjs` + `POST /api/v1/notes/:path/upload-image` with triple file validation; (E) Hub paste-URL helper with preview + upload; (F) Hosted parity — gateway handles upload directly (no binary Lambda hop), image-proxy uses GitHub Contents API `download_url` redirect (no response-size limit), client-side Canvas compression to ≤3 MB before upload, `apiBase`-prefixed fetch URLs so requests from ICP-hosted frontend reach the Netlify gateway. Shared: `lib/media-url-extract.mjs`, `hub/gateway/parse-multipart.mjs`. 515 total tests passing.
...
**What we do *not* do in the thin bridge:** Replace the canister, require Muse for proposals, or implement the full **Knowtation domain plugin** (snapshot/diff/merge inside Muse) — that stays **Option A** / deferred until a concrete need. **Full plugin:** large optional effort; **thin bridge:** [MUSE-THIN-BRIDGE.md](./MUSE-THIN-BRIDGE.md) and [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §4.

**Tasks (when prioritized):**

- [x] Document thin bridge for operators: [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §4 (*Optional external lineage*); operator runbook [MUSE-THIN-BRIDGE.md](./MUSE-THIN-BRIDGE.md).
...
7. ~~**MCP D2/D3, F2–F5**~~ — ✅ Complete + merged (PR #101) + **AWS gateway deployed** (EC2 `18.221.120.124`, PM2 + Nginx, OAuth 2.1 live). **Issue #2** deferred.
8. ~~**Phase 18 (Media URL + MCP image/video resources)**~~ — ✅ Done (`feature/phase18-image-resources`). 5 scope items (A–E), 4 new files, 4 modified, 83 new tests.
...
- **Issue #1 Phase C (enhanced tools):** `mcp/tools/phase-c.mjs` — relate, backlinks, capture, transcribe, vault_sync, summarize, extract_tasks, cluster, memory_query, tag_suggest; libs under `lib/`.
...
7. **Docs** — When to use hub vs Git+PRs; how to run self-hosted hub (Docker) and use hosted Hub (ICP); how to get tokens and use the API. Link to [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §4, [HUB-API.md](./HUB-API.md), and [PROPOSAL-LIFECYCLE.md](./PROPOSAL-LIFECYCLE.md).
...
- **Simple agent-to-agent and agent-to-human:** Phase 11 (shared vault / hub) — API, proposals, review queue, optional UI — so people who are unfamiliar with or adverse to GitHub can still share a vault and review proposals. Optional; core remains usable without it. See [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §4.
...
- **Muse Option A (full domain plugin / Muse as variation backend):** Out of scope unless there is a concrete need (partner, shared DAG, structural merge ownership). See [MUSE-THIN-BRIDGE.md](./MUSE-THIN-BRIDGE.md) and [AGENT-INTEGRATION.md](./AGENT-INTEGRATION.md) §4.
