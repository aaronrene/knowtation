...
| — | **Hosted / ICP (canister)** | Code done | Canister (hub/icp/), gateway (hub/gateway/), bridge (hub/bridge/); single URL knowtation.store; DEPLOY-HOSTED, CANISTER-AND-SINGLE-URL. **Deploy pending:** dfx, 4Everland, Netlify, DNS. **Parity pending:** roles, invites, POST setup (gateway stubs per [PARITY-PLAN.md](./PARITY-PLAN.md)). See [STATUS-HOSTED-AND-PLANS.md](./STATUS-HOSTED-AND-PLANS.md). |
