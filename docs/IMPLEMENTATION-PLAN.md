...
| **Then (hosted)** | **Team vault access + scope:** Same semantics as self-hosted **`hub_vault_access.json`** / **`hub_scope.json`** for **other users** on hosted (not yet on the canister path — see §2.1 snapshot). | Planned **next** hosted parity slice after 15.1 verify; shapes API + canister/bridge design. |
| **Then (hosted)** | **Hub Import** on hosted (replace gateway **501**; multipart import to canister path). | [PARITY-PLAN.md](./PARITY-PLAN.md). Typically **after** access/scope unless product reorders. |
...
3. **Multi-vault on hosted:** Phase 15.1 **partition + UI bootstrap** merged (**#46–#48**). **Remaining gap vs self-hosted:** team **vault access** + **scope** on hosted, then **Hub Import**; **ops:** redeploy + §5.1 when needed.
...
- **Issue #1 Phase D1 (Streamable HTTP):** `mcp/http-server.mjs`, `mcp/create-server.mjs` (shared mount), `mcp/stdio-main.mjs`, `mcp/server.mjs` transport switch; `config.mcp.http_port` / `http_host`; `express` dependency. D2/D3 (Hub + OAuth) not implemented.
...
- **Issue #1 Phase F1 (sampling — summarize):** `mcp/tools/phase-c.mjs` — when the MCP client advertises `sampling`, `summarize` uses `Server#createMessage` (`sampling/createMessage`); otherwise existing `completeChat` (Ollama/OpenAI). F2–F5 backlog in [BACKLOG-MCP-SUPERCHARGE.md](./BACKLOG-MCP-SUPERCHARGE.md).
...
3. **Beta disclaimer** — On landing or Hub: hosted is beta; **usage is open** while we **research** real-world patterns; when billing goes live, **card subscriptions** (three tiers) with **included usage**; **going over** means **upgrading** tier, with **optional credit top-ups** planned **soon** for flexibility (see [HOSTED-CREDITS-DESIGN.md](./HOSTED-CREDITS-DESIGN.md)).
...
## Phase 16 — Hosted billing: subscriptions + included credits + rollover add-ons (optional)

**Goal:** **Hosted** billing follows a **Netlify-style hybrid** ([HOSTED-CREDITS-DESIGN.md](./HOSTED-CREDITS-DESIGN.md)): **$0 Free** tier (small monthly allowance) + **Stripe** subscriptions (**Starter / Pro / Team**) with **monthly included** credits (**1 credit = $1** internal meter, transparent **per-action** prices in API), plus **rollover add-on packs**. **Metering hooks** + optional **`BILLING_SHADOW_LOG`** for research. **Beta:** `BILLING_ENFORCE` off by default. **Future:** usage **history + chart** in Hub (documented, not launch-blocking). **No crypto** in the first slice. Optional **grandfather** early users.

**Design docs (authoritative for product shape):**

- [HOSTED-STORAGE-BILLING-ROADMAP.md](./HOSTED-STORAGE-BILLING-ROADMAP.md) — **Multi-vault** V1 + reserved **`monthly_*` / `addon_cents`** (or gateway-only store until canister mirrors).
- [HOSTED-CREDITS-DESIGN.md](./HOSTED-CREDITS-DESIGN.md) — v0 price table, dual pools, Stripe webhooks, Hub UX, **`402`** codes.

**Deliverables (implementation):**

1. **Gateway billing store** — Per `user_id`: `tier`, `period_*`, `monthly_included_cents`, `monthly_used_cents`, `addon_cents`, Stripe ids; **`data/hosted_billing.json`** locally, **Netlify Blob** in production (`netlify/functions/gateway.mjs`).
2. **Stripe** — **Subscriptions** + **Customer Portal** + **Checkout** for **packs**; **`POST /api/v1/billing/webhook`** (raw body); **idempotent** `event.id` ledger.
3. **Consumption order** — Deduct **monthly included first**, then **`addon_cents`**; shared helper in `hub/gateway/billing-logic.mjs`.
4. **Enforcement** — When `BILLING_ENFORCE=true`, gateway middleware on **bridge-proxied** search/index and **canister-proxied** writes (extend as needed); **`402`** + `QUOTA_EXHAUSTED` when both pools insufficient.
5. **`GET /api/v1/billing/summary`** — JWT; pools, **`cost_breakdown`** (labels + USD), **`credit_policy`**, **`usage_chart_status`** roadmap field.
6. **Shadow metering** — `BILLING_SHADOW_LOG=true`: one JSON line per billable op (`knowtation_billing_shadow`) for aggregations / COGS research.
7. **Hub UI (follow-on)** — Balances, per-action **transparency** from `cost_breakdown`; **Buy credits** / Portal; later **usage graph** from stored events.

**Acceptance:** Webhooks update state; summary reflects pools; with enforcement on, over-quota operations return **402**; add-on purchases increase **rollover** balance. Self-hosted unchanged.

**Depends on:** Phase 11, 14; **Phase 15.1** canister migration optional for billing **until** you mirror state on-chain. Order: **gateway billing + Stripe envs → Hub UI → optional canister fields per roadmap.**
