...
**Strategic sequencing (hosted product — 2026-03):** **Hosted parity for import** (`POST /api/v1/import` replacing gateway **501**) is the **next product gate** before investing in **Stripe checkout, subscription UX, and billing enforcement**. Reason: users must be able to **load vault content** on the hosted path; metering paid usage without import is cart-before-horse. **Branch:** implement on **`feature/hosted-import-parity`** (or follow-on). Indexing-token **telemetry** and **`billing/summary`** may already be on `main` after the billing PR merge; **hard quotas and Customer Portal** come **after** import ships. See [HOSTED-IMPORT-DESIGN.md](./HOSTED-IMPORT-DESIGN.md), [PARITY-PLAN.md](./PARITY-PLAN.md).
...
| **P0 (hosted)** | **Hub Import** on hosted (replace gateway **501**; multipart → canister/bridge path). **Before** Stripe/billing completion. Branch **`feature/hosted-import-parity`**. | [HOSTED-IMPORT-DESIGN.md](./HOSTED-IMPORT-DESIGN.md), [PARITY-PLAN.md](./PARITY-PLAN.md). |
...
| **Later** | **Phase 16 (completion):** **Stripe** Checkout/subs, **Customer Portal**, pack webhooks, **period reset**, **402** token enforcement — **after** **hosted import** ships ([HOSTED-IMPORT-DESIGN.md](./HOSTED-IMPORT-DESIGN.md)). Telemetry may already exist on `main`. **Phase 12:** Blockchain when needed. **MCP D2/D3, F2–F5** per backlog. | After hosted import parity. |
...
2. **Hosted import (P0):** Replace gateway **501** on **`POST /api/v1/import`** — **before** Stripe subscription UX and billing enforcement. **Branch:** `feature/hosted-import-parity`. **Spec:** [HOSTED-IMPORT-DESIGN.md](./HOSTED-IMPORT-DESIGN.md), [PARITY-PLAN.md](./PARITY-PLAN.md).
3. **Billing (Phase 16 completion):** After import parity — Stripe products, webhooks, pack balance, period reset, optional **402** enforcement. **Design:** [HOSTED-CREDITS-DESIGN.md](./HOSTED-CREDITS-DESIGN.md). Indexing-token **recording + Hub Billing tab** may already be on `main` from the billing merge.
4. **Multi-vault on hosted:** Phase 15.1 **partition + UI bootstrap** merged (**#46–#48**); **team vault access + scope** in repo ([HOSTED-WORKSPACE-ACCESS.md](./HOSTED-WORKSPACE-ACCESS.md)). **Ops:** redeploy gateway + bridge + §5.1 when needed.
5. **Memory:** Compare **file memory (Phase 8)** vs **Mem0 import/API** on the same vault; pick one primary story for docs and MCP until requirements firm up.
6. **Evaluation stage:** Design **status transitions** and permissions (who runs evaluation, who can approve after pass/fail). Implement after the lifecycle is written down.
7. **Muse thin bridge:** Docs + optional env and small delegation surface; **full Muse domain plugin** remains deferred until a concrete partner or DAG need appears.
8. **Phase 12 (blockchain / wallets) — optional:** **Read-only** chain context + MCP/tools per [BLOCKCHAIN-AND-AGENT-PAYMENTS.md](./BLOCKCHAIN-AND-AGENT-PAYMENTS.md); **after** hosted core + import confidence unless a partner forces earlier scope.
