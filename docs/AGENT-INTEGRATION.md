...
- **Summarize arbitrary pasted blob (hosted):** Evaluated and **not shipped** — see [`MCP-BLOB-SUMMARIZE-EVALUATION.md`](./MCP-BLOB-SUMMARIZE-EVALUATION.md). Use **`summarize`** on a **vault path**, the **`meeting-notes`** prompt for pasted transcripts, or **local** summarization for terminal output (per [`WHY-KNOWTATION.md`](./WHY-KNOWTATION.md) token layers).
