...
- **Session management:** Each authenticated user gets an isolated MCP session with role-based tool access (viewer: read-only; editor: + write; admin: + index/export/import). **Idle TTL** defaults to **8 hours** (was 30 minutes) so remote clients (Cursor) keep tools/resources across breaks; override with **`MCP_SESSION_TTL_MS`** (milliseconds, clamped 5m–24h). **`MCP_MAX_SESSIONS_PER_USER`** caps concurrent sessions per user (default **8**, clamped 2–20; oldest evicted when exceeded). On each **`/mcp`** request, the gateway copies the current **`Authorization`** / **`X-Vault-Id`** into the session’s **`liveAuth`** so canister/bridge calls use a **refreshed JWT**, not only the token captured at first connect (this reduces “empty tools / missing prompts” after access-token expiry while the MCP session is still alive).
- **Prompts (Track B1 + B2):** On **`main`**, **`prompts/list`** includes five B1 prompts (read-only) that compose **`list_notes` / `search` / `get_note`**. Branch **`feat/hosted-mcp-prompts-b2`** adds five more (`meeting-notes`, `knowledge-gap`, `causal-chain`, `extract-entities`, `write-from-capture`); **`write-from-capture`** requires **editor** role; others match viewer-gated tools. **`maybeAppendSamplingPrefill`** may run on **`knowledge-gap`** and the B1 prompts that already used it; without sampling, message lists still return without a draft assistant turn.
...
- **EC2 / VPS deploy (clone, `git pull`, `npm ci`, PM2):** Step-by-step operator runbook, dual-host env matrix, and Nginx notes: [DEPLOY-HOSTED.md §3.1](./DEPLOY-HOSTED.md#ec2-mcp-gateway-runbook).
