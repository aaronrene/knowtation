...
|--------|-------------|---------------------------|-------|
| `search <query>` | Semantic (and optional hybrid) search over indexed vault. | `--folder <path>`, `--project <slug>`, `--tag <tag>`, `--limit <n>` (default 10), `--since <date>`, `--until <date>`, `--chain <id>`, `--entity <id>`, `--episode <id>`, `--order date\|date-asc`, `--fields path\|path+snippet\|full` (default path+snippet), `--snippet-chars <n>`, `--count-only`, `--json` | Returns ranked notes/chunks. Time and causal filters optional; see docs/INTENTION-AND-TEMPORAL.md. Token levers: see docs/RETRIEVAL-AND-CLI-REFERENCE.md. |
...
- **search (--json):**
  - Default or `--fields path+snippet`: `{ "results": [ { "path": "...", "snippet": "...", "score": number, "project": "...", "tags": [] } ], "query": "..." }`. Snippet length may be capped by `--snippet-chars <n>`.
