...
export function resolveStarterFlowsDir(moduleUrl) {
  const cwdFallback = path.join(getRepoRoot(), STARTER_FLOWS_DIRNAME);
  if (moduleUrl) {
    const base = path.dirname(fileURLToPath(moduleUrl));
    const fromModule = path.normalize(path.join(base, ...STARTER_REL_FROM_MODULE));
...
export function getVaultFlowStore(dataDir, vaultId) {
  ...
  if (!Array.isArray(store.vaults[vaultId].learning_paths)) {
    store.vaults[vaultId].learning_paths = [];
  }
