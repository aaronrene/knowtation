...
export function seedStarterFlows(dataDir, vaultId, options = {}) {
  ...
  for (const file of files) {
    ...
    for (const step of steps) {
      const stepExists = vault.steps.some((s) => s.step_id === step.step_id);
      if (!stepExists) {
 ...
 * @returns {number}
 */
function countStepsForFlow(vault, flowId) {
  return vault.steps.filter((s) => s.flow_id === flowId).length;
}
...
export function listFlows(dataDir, vaultId, query) {
  ...
  const flows = candidates.map((flow) => flowSummaryForClient(flow, countStepsForFlow(vault, flow.flow_id)));
...
export function latestStoredFlow(vault, flowId) {
  ...
  const steps = vault.steps
    .filter((s) => s.flow_id === flowId)
 ...
 * versions stay pinnable. Because the 7A-10b store keys step bodies by `step_id`
 * (= `flow_id#ordinal`) only — not `(step_id, version)` — step bodies for a
 * `flow_id` are shared across versions; the new version's steps replace that
 * shared set (a versioned-step-keying slice is recommended at 7A-10c). Writes
 * atomically (tmp + rename) so a failed reconcile leaves zero partial state.
 *
 * @param {string} dataDir
 * @param {string} vaultId
...
export function upsertFlowVersion(dataDir, vaultId, flow, steps) {
  ...
  vault.steps = vault.steps.filter((s) => s.flow_id !== flow.flow_id);
  for (const step of steps) {
    vault.steps.push(step);
  }
