 ...
 * @returns {object|null}
 */
function findVisibleRun(vault, runId, visibleScopes) {
  if (!vault || !Array.isArray(vault.runs)) return null;
  const run = vault.runs.find((r) => r.run_id === runId);
  if (!run) return null;
 ...
 * @returns {object|null}
 */
function findStepDefinition(vault, flowId, stepId) {
  if (!vault || !Array.isArray(vault.steps)) return null;
  return vault.steps.find((s) => s.flow_id === flowId && s.step_id === stepId) ?? null;
}
...
export function handleFlowRunListRequest(input) {
  ...
  const filtered = runs.filter((r) => {
    if (!resolved.visibleScopes.has(r.scope)) return false;
...
export function handleFlowRunAdvanceRequest(input) {
  ...
  const stepDef = findStepDefinition(vault, run.flow_id, stepId);
  if (!stepDef) {
 ...
 * @returns {{ ok: true, payload: object } | { ok: false, status: number, error: string, code: string }}
 */
export function handleFlowRunSubmitReviewRequest(input) {
  if (getFlowExecutionPolicyForbidden(input.dataDir)) {
    return refuse(403, 'FLOW_EXECUTION_POLICY_FORBIDDEN', 'Execution forbidden by policy');
  }
  if (!getFlowRunWritesEnabled(input.dataDir)) {
  ...
  const proposal = input.createProposal(input.dataDir, {
    ...
    body: JSON.stringify({ run_id: run.run_id, flow_id: run.flow_id, flow_version: run.flow_version }, null, 2),
    frontmatter: {
 ...
 * @returns {{ ok: true, payload: object } | { ok: false, status: number, error: string, code: string }}
 */
export function handleFlowRunMcpRequest(input) {
