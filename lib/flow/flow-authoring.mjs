 ...
 *   createProposal: (dataDir: string, input: object) => { proposal_id: string } | Promise<{ proposal_id: string }>,
 *   starterDir?: string,
 * }} input
 * @returns {Promise<{ ok: true, payload: object } | { ok: false, status: number, error: string, code: string }>}
 */
export async function handleFlowProposeRequest(input) {
  ...
  // FLOW-WRITE-LIVE §FWL.4.1 — optional scooling.flow: ref (malformed → 400; absent → ok).
  // Import lineage hints (sourceVaultHint / free-form muse refs) MUST NOT substitute.
  const refGate = resolveFlowProposeExternalRef(input);
  if (!refGate.ok) return refGate;
  ...
  const proposal = await Promise.resolve(
    input.createProposal(input.dataDir, {
      ...
      ...(refGate.externalRef ? { external_ref: refGate.externalRef } : {}),
...
/**
 * Optional Scooling Flow `external_ref` on propose (§FWL.4.1).
 * Malformed → 400; absent → ok/undefined (propose may succeed; not admitted).
 *
 * @param {object} input
 * @returns {{ ok: true, externalRef: string|undefined } | { ok: false, status: number, error: string, code: string }}
 */
function resolveFlowProposeExternalRef(input) {
