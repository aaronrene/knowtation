 ...
 * }} input
 */
export function handleFlowCaptureProposeRequest(input) {
 ...
 * }} input
 */
export function handleFlowCaptureDismissRequest(input) {
  if (!getFlowCaptureWritesEnabled(input.dataDir)) {
    return refuse(403, 'FLOW_CAPTURE_WRITES_DISABLED', 'Flow capture writes are disabled');
  }
  ...
  const proposal = input.createProposal(input.dataDir, {
    path: `meta/candidates/${candidateId}.md`,
    body,
    frontmatter: { type: 'flow_capture', candidate_id: candidateId, proposal_kind: 'flow_candidate_dismiss' },
    intent,
    base_state_id: absentFlowStateId(),
    source: FLOW_CAPTURE_PROPOSAL_SOURCE,
    vault_id: input.vaultId,
