...
/** Parent segments from any package module to bundled `tasks/starter` (hub/bridge, lib/task, …). */
const STARTER_REL_FROM_MODULE = ['..', '..', 'tasks', 'starter'];

/**
 * Resolve bundled starter task JSON directory (Netlify Lambda cwd-safe).
 *
 * @param {string | URL} [moduleUrl] `import.meta.url` of a module in this package
 * @returns {string}
 */
export function resolveStarterTasksDir(moduleUrl) {
  if (moduleUrl) {
    const base = path.dirname(fileURLToPath(moduleUrl));
    return path.normalize(path.join(base, ...STARTER_REL_FROM_MODULE));
  }
  return path.join(getRepoRoot(), STARTER_TASKS_DIRNAME);
}
...
/** Agent queue assignee (Phase 7D external protocol). */
export const AGENT_ASSIGNEE_REF_RE = /^agent_[a-z0-9_]{1,64}$/;
 ...
 * @returns {boolean}
 */
export function isValidTaskAssigneeRef(ref) {
  if (ref == null) return true;
  if (typeof ref !== 'string') return false;
  if (ref === '*') return true;
  return UID_HASH_REF_RE.test(ref) || AGENT_ASSIGNEE_REF_RE.test(ref);
}
...
/** @type {readonly ['observe_only', 'draft_only', 'propose_only', 'execute_with_consent']} */
 ...
 * @property {string|null} [external_pass_id]
 * @property {string|null} [external_lease_expires_at]
 * @property {string|null} [external_active_grant_id]
...
export function validateTaskRecord(raw) {
  ...
  if (!isValidTaskAssigneeRef(task.assignee_ref)) {
    return { ok: false, reason: 'assignee_ref must be uid_hash:<64-hex>, agent_<id>, *, or null' };
  }
  ...
  if (task.external_pass_id != null && typeof task.external_pass_id !== 'string') {
    return { ok: false, reason: 'external_pass_id must be string or null' };
  }
  if (task.external_lease_expires_at != null && !isIso8601(task.external_lease_expires_at)) {
    return { ok: false, reason: 'external_lease_expires_at must be ISO8601 UTC or null' };
  }
  if (task.external_active_grant_id != null && typeof task.external_active_grant_id !== 'string') {
    return { ok: false, reason: 'external_active_grant_id must be string or null' };
  }
  const boundaryPolicyRaw =
    task.boundary_policy == null || task.boundary_policy === undefined
      ? 'observe_only'
      : task.boundary_policy;
  if (
    !BOUNDARY_POLICIES.includes(/** @type {typeof BOUNDARY_POLICIES[number]} */ (boundaryPolicyRaw))
  ) {
  ...
  return {
    ...
    task: {
      ...
      external_pass_id: task.external_pass_id == null ? null : String(task.external_pass_id),
      external_lease_expires_at: task.external_lease_expires_at == null ? null : String(task.external_lease_expires_at),
      external_active_grant_id: task.external_active_grant_id == null ? null : String(task.external_active_grant_id),
      boundary_policy: /** @type {typeof BOUNDARY_POLICIES[number]} */ (boundaryPolicyRaw),
    },
  };
}
...
/**
 * Seed SD-2 anchor run when absent (read-only; delegates to flow-store P-FLOW seed).
 *
 * @param {string} dataDir
 * @param {string} vaultId
...
export function seedSd2AnchorRun(dataDir, vaultId) {
  return seedOverseerAnchorRun(dataDir, vaultId);
}
