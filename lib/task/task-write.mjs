...
function commonProposeGate(input) {
  ...
  const ext = resolveTaskProposeExternalRef(input);
  if (!ext.ok) return ext;
  return { ok: true, intent, visibleScopes: resolved.visibleScopes, externalRef: ext.externalRef };
}

/**
 * Support sync (self-hosted) and async (hosted canister) createProposal injectors.
 *
 * @param {object} input
...
/**
 * Optional Scooling task external_ref on propose (§FCA.4.1). Malformed → 400; absent → ok.
 *
 * @param {object} input
 * @returns {{ ok: true, externalRef: string|undefined } | ReturnType<typeof refuse>}
 */
function resolveTaskProposeExternalRef(input) {
 ...
 */
export async function handleTaskProposeRequest(input) {
  ...
  if (proposalKindRaw === 'task_create') {
    return await handleTaskCreatePropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
  if (proposalKindRaw === 'task_status_update') {
    return await handleTaskStatusUpdatePropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
  if (proposalKindRaw === 'task_assign') {
    return await handleTaskAssignPropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
  if (proposalKindRaw === 'task_artifact_link') {
    return await handleTaskArtifactLinkPropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
 ...
 * @param {string} intent
 * @param {Set<TaskScope>} visibleScopes
 * @param {string|undefined} externalRef
 */
async function handleTaskCreatePropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskStatusUpdatePropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  ...
  const proposalBody = JSON.stringify(
    { proposal_kind: 'task_status_update', task_id: taskId, scope: existing.scope, ...patch },
    null,
    2,
  );
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskAssignPropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const taskId = typeof body.task_id === 'string' ? body.task_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
  ...
  if (!isValidTaskAssigneeRef(assigneeRef)) {
    return refuse(400, 'TASK_DRAFT_INVALID', 'assignee_ref must be uid_hash:<64-hex>, agent_<id>, *, or null');
  }
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskArtifactLinkPropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const taskId = typeof body.task_id === 'string' ? body.task_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
  ...
  const proposalBody = JSON.stringify(
    {
      ...
      task_id: taskId,
      scope: existing.scope,
      artifact_link: { kind: row.kind, ref: row.ref },
    },
    null,
    2,
  );
 ...
 * @returns {{ ok: true, payload: object } | ReturnType<typeof refuse>}
 */
export async function handleTaskLoopProposeRequest(input) {
  const gate = commonProposeGate(input);
  if (!gate.ok) return gate;
  ...
  if (proposalKindRaw === 'task_loop_create') {
    return await handleTaskLoopCreatePropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
  if (proposalKindRaw === 'task_loop_pause') {
    return await handleTaskLoopPausePropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
  if (proposalKindRaw === 'task_loop_cancel') {
    return await handleTaskLoopCancelPropose(input, gate.intent, gate.visibleScopes, gate.externalRef);
  }
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopCreatePropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopPausePropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  ...
  const proposalBody = JSON.stringify(
    { proposal_kind: 'task_loop_pause', loop_id: loopId, scope: existing.scope },
    null,
    2,
  );
  const proposal = await createProposalRecord(input, {
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopCancelPropose(input, intent, visibleScopes, externalRef) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const loopId = typeof body.loop_id === 'string' ? body.loop_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
  ...
  const proposalBody = JSON.stringify(
    { proposal_kind: 'task_loop_cancel', loop_id: loopId, scope: existing.scope },
    null,
    2,
  );
  const proposal = await createProposalRecord(input, {
 ...
 * @returns {{ ok: true, payload: object } | ReturnType<typeof refuse>}
 */
export async function handleTaskInstanceMaterializeRequest(input) {
  const gate = commonProposeGate(input);
  if (!gate.ok) return gate;
  ...
  const proposal = await createProposalRecord(input, {
    ...
    proposed_by: typeof input.userId === 'string' && input.userId.trim() ? input.userId.trim() : undefined,
    review_queue: TASK_REVIEW_QUEUE,
    ...(gate.externalRef ? { external_ref: gate.externalRef } : {}),
    task_meta: {
