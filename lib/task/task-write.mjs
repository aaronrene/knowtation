...
async function createProposalRecord(input, proposalInput) {
  return await Promise.resolve(input.createProposal(input.dataDir, proposalInput));
}
...
export async function handleTaskProposeRequest(input) {
  ...
  if (proposalKindRaw === 'task_create') {
    return await handleTaskCreatePropose(input, gate.intent, gate.visibleScopes);
  }
  if (proposalKindRaw === 'task_status_update') {
    return await handleTaskStatusUpdatePropose(input, gate.intent, gate.visibleScopes);
  }
  if (proposalKindRaw === 'task_assign') {
    return await handleTaskAssignPropose(input, gate.intent, gate.visibleScopes);
  }
  if (proposalKindRaw === 'task_artifact_link') {
    return await handleTaskArtifactLinkPropose(input, gate.intent, gate.visibleScopes);
  }
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskCreatePropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskStatusUpdatePropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskAssignPropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const taskId = typeof body.task_id === 'string' ? body.task_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskArtifactLinkPropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const taskId = typeof body.task_id === 'string' ? body.task_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
...
export async function handleTaskLoopProposeRequest(input) {
  ...
  if (proposalKindRaw === 'task_loop_create') {
    return await handleTaskLoopCreatePropose(input, gate.intent, gate.visibleScopes);
  }
  if (proposalKindRaw === 'task_loop_pause') {
    return await handleTaskLoopPausePropose(input, gate.intent, gate.visibleScopes);
  }
  if (proposalKindRaw === 'task_loop_cancel') {
    return await handleTaskLoopCancelPropose(input, gate.intent, gate.visibleScopes);
  }
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopCreatePropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopPausePropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
 ...
 * @param {Set<TaskScope>} visibleScopes
 */
async function handleTaskLoopCancelPropose(input, intent, visibleScopes) {
  const body = input.body && typeof input.body === 'object' ? input.body : {};
  const loopId = typeof body.loop_id === 'string' ? body.loop_id.trim() : '';
  const baseStateId = typeof body.base_state_id === 'string' ? body.base_state_id.trim() : '';
