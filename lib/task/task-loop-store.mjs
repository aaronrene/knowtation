...
import { validateTaskLoopRrule } from './task-loop-rrule.mjs';
...
/** Parent segments from any package module to bundled starter dirs (hub/bridge, lib/task, …). */
const STARTER_LOOPS_REL_FROM_MODULE = ['..', '..', 'task-loops', 'starter'];
const STARTER_GRAPHS_REL_FROM_MODULE = ['..', '..', 'orchestrator-graphs', 'starter'];
const STARTER_INSTANCES_REL_FROM_MODULE = ['..', '..', 'task-loops', 'starter', 'instances'];

/**
 * Resolve bundled starter task-loop JSON directory (Netlify Lambda cwd-safe).
 *
 * @param {string | URL} [moduleUrl] `import.meta.url` of a module in this package
 * @returns {string}
 */
export function resolveStarterTaskLoopsDir(moduleUrl) {
  if (moduleUrl) {
    const base = path.dirname(fileURLToPath(moduleUrl));
    return path.normalize(path.join(base, ...STARTER_LOOPS_REL_FROM_MODULE));
  }
  return path.join(getRepoRoot(), STARTER_TASK_LOOPS_DIRNAME);
}

/**
 * Resolve bundled orchestrator graph starter directory (Netlify Lambda cwd-safe).
 *
 * @param {string | URL} [moduleUrl]
 * @returns {string}
 */
export function resolveStarterOrchestratorGraphsDir(moduleUrl) {
  if (moduleUrl) {
    const base = path.dirname(fileURLToPath(moduleUrl));
    return path.normalize(path.join(base, ...STARTER_GRAPHS_REL_FROM_MODULE));
  }
  return path.join(getRepoRoot(), STARTER_ORCHESTRATOR_GRAPHS_DIRNAME);
}

/**
 * Resolve bundled loop instance task starter directory (Netlify Lambda cwd-safe).
 *
 * @param {string | URL} [moduleUrl]
 * @returns {string}
 */
export function resolveStarterLoopInstancesDir(moduleUrl) {
  if (moduleUrl) {
    const base = path.dirname(fileURLToPath(moduleUrl));
    return path.normalize(path.join(base, ...STARTER_INSTANCES_REL_FROM_MODULE));
  }
  return path.join(getRepoRoot(), STARTER_LOOP_INSTANCES_DIRNAME);
}
 ...
 */
export function validateRecurrence(raw, seriesTimezone) {
  ...
  if (kind === 'rrule') {
    if (typeof seriesTimezone !== 'string' || !seriesTimezone.trim()) {
      return { ok: false, reason: 'rrule recurrence requires series timezone context' };
    }
    const rruleResult = validateTaskLoopRrule(recurrence, seriesTimezone);
    if (!rruleResult.ok) {
      return { ok: false, reason: `invalid rrule: ${rruleResult.detail}` };
    }
    return { ok: true, recurrence: rruleResult.recurrence };
  }
...
export function validateTaskLoopRecord(raw) {
  ...
  const recurrenceResult = validateRecurrence(loop.recurrence, String(loop.timezone));
  ...
  let triggerBindings;
  if (loop.trigger_bindings != null) {
    if (!Array.isArray(loop.trigger_bindings)) {
      return { ok: false, reason: 'trigger_bindings must be an array' };
    }
    triggerBindings = [];
    for (const bindingRaw of loop.trigger_bindings) {
      if (!bindingRaw || typeof bindingRaw !== 'object') {
        return { ok: false, reason: 'each trigger_binding must be an object' };
      }
      const binding = /** @type {Record<string, unknown>} */ (bindingRaw);
      if (binding.schema !== 'knowtation.loop_trigger_binding/v0') {
        return { ok: false, reason: 'invalid trigger_binding schema' };
      }
      if (typeof binding.binding_id !== 'string' || !/^bind_[a-z0-9_]{1,48}$/.test(binding.binding_id)) {
        return { ok: false, reason: 'invalid binding_id' };
      }
      const signalKind = binding.signal_kind;
      if (
        signalKind !== 'manual' &&
        signalKind !== 'task_status' &&
        signalKind !== 'calendar_timeline' &&
        signalKind !== 'note_edit'
      ) {
        return { ok: false, reason: 'invalid signal_kind' };
      }
      if (typeof binding.debounce_seconds !== 'number' || binding.debounce_seconds < 1 || binding.debounce_seconds > 86400) {
        return { ok: false, reason: 'invalid debounce_seconds' };
      }
      ...
      if (!match || typeof match !== 'object') {
        return { ok: false, reason: 'match required on trigger_binding' };
      }
      const matchRow = /** @type {Record<string, unknown>} */ (match);
      const matchKind = matchRow.kind;
      if (matchKind === 'note_path' && typeof matchRow.path === 'string') {
        if (matchRow.path.includes('..') || matchRow.path.startsWith('/')) {
          return { ok: false, reason: 'invalid note_path in trigger_binding' };
        }
      } else if (matchKind === 'folder_prefix' && typeof matchRow.prefix === 'string') {
        if (matchRow.prefix.includes('..') || matchRow.prefix.startsWith('/')) {
          return { ok: false, reason: 'invalid folder_prefix in trigger_binding' };
        }
      }
      triggerBindings.push({
        schema: 'knowtation.loop_trigger_binding/v0',
        binding_id: binding.binding_id,
        signal_kind: signalKind,
        match,
        debounce_seconds: binding.debounce_seconds,
