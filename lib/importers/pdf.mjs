...
async function extractPdfBytes(buf) {
  try {
    const bytes = Buffer.isBuffer(buf) ? buf : Buffer.from(buf);
    const pdf = await getDocumentProxy(new Uint8Array(bytes));
    const { totalPages, text } = await extractText(pdf, { mergePages: true });
    const markdown = normalizeExtractedText(text);
    return markdown ? { ok: true, markdown, totalPages } : { ok: false, reason: 'empty_extract' };
  } catch {
...
/**
 * Convert PDF bytes to normalized Markdown without writing a note.
 *
 * @param {Buffer|Uint8Array} buf
 * @returns {Promise<{ ok: true, markdown: string } | { ok: false, reason: 'empty_extract' }>}
 */
export async function pdfBytesToMarkdown(buf) {
  const result = await extractPdfBytes(buf);
  return result.ok
    ? { ok: true, markdown: result.markdown }
    : { ok: false, reason: 'empty_extract' };
}
...
export async function importPdf(input, ctx) {
  ...
  const extracted = await extractPdfBytes(buf);
  if (!extracted.ok) {
    throw new Error('Could not extract text from this PDF (empty or image-only)');
  }
  const bodyText = extracted.markdown;
