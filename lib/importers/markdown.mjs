...
function isMarkdownFilename(nameOrPath) {
...
export async function importMarkdown(input, ctx) {
  ...
  if (fs.statSync(absInput).isFile()) {
    if (isMarkdownFilename(absInput)) files.push({ fullPath: absInput, relPath: path.basename(absInput) });
  } else {
...
function walkMarkdown(rootDir, dir, relDir, out) {
  ...
  for (const e of entries) {
    ...
    } else if (isMarkdownFilename(e.name)) {
