...
async function convertDocxBytes(buf) {
  try {
    const result = await mammoth.convertToMarkdown({
      buffer: Buffer.isBuffer(buf) ? buf : Buffer.from(buf),
    });
    const markdown = normalizeMarkdownBody(result.value);
    return markdown
      ? { ok: true, markdown, messages: result.messages ?? [] }
...
/**
 * Convert DOCX bytes to normalized Markdown without writing a note.
 *
 * @param {Buffer|Uint8Array} buf
 * @returns {Promise<{ ok: true, markdown: string } | { ok: false, reason: 'empty_extract' }>}
 */
export async function docxBytesToMarkdown(buf) {
  const result = await convertDocxBytes(buf);
  return result.ok
