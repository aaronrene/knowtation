...
const MAX_JSON_BLOCK_CHARS = 512_000;
 ...
 */
export function buildRowObjectForJson(headers, cells) {
  /** @type {Record<string, string>} */
  const o = {};
  for (let c = 0; c < headers.length; c++) {
    let base = (headers[c] && String(headers[c]).trim()) || `column_${c}`;
    let key = base;
    let n = 2;
    while (Object.hasOwn(o, key)) {
      key = `${base}__${n}`;
      n++;
    }
    let v = c < cells.length ? String(cells[c] ?? '') : '';
...
export async function importStringMatrixToNotes(matrix, ctx, meta) {
  ...
  for (let rowNum = 1; rowNum < matrix.length; rowNum++) {
    ...
    const rowObj = buildRowObjectForJson(headers, cells);
    let jsonBlock = JSON.stringify(rowObj, null, 2);
    if (jsonBlock.length > MAX_JSON_BLOCK_CHARS) {
      jsonBlock = jsonBlock.slice(0, MAX_JSON_BLOCK_CHARS) + '\n…(truncated)';
    }
    bodyLines.push('', '## Full row (JSON)', '', '```json', jsonBlock, '```');
    ...
    const frontmatter = {
      ...
      [fileKey]: fileLabel,
      row_index: rowNum,
      import_column_headers: JSON.stringify(headers),
