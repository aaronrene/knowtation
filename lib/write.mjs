 ...
 *
 * When `options.config` is provided and `config.air.enabled=true`, attestation is obtained via
 * `attestBeforeWrite` before the file is written. A real (non-placeholder) attestation ID is
 * stored as `air_id` in the note's frontmatter (Improvement A). If `air.required=true` and
 * attestation fails, an `AttestationRequiredError` is thrown and no file is written.
 *
 * @param {string} vaultPath - Absolute path to vault root
 * @param {string} relativePath - Vault-relative path (e.g. inbox/foo.md)
 * @param {{ body?: string, frontmatter?: Record<string, string>, append?: boolean, config?: object }} options
 * @returns {Promise<{ path: string, written: boolean }>}
 * @throws if path escapes vault, write fails, or attestation is required but unavailable
 */
export async function writeNote(vaultPath, relativePath, options = {}) {
  const safe = resolveVaultRelativePath(vaultPath, relativePath);
  const fullPath = path.join(vaultPath, safe);
  ...
  // Improvement A: obtain attestation and store returned id in frontmatter.
  // attestBeforeWrite handles the enabled/inbox/required checks internally.
  if (options.config) {
    const { attestBeforeWrite } = await import('./air.mjs');
    const airId = await attestBeforeWrite(options.config, safe.replace(/\\/g, '/'));
    if (airId && airId !== 'air-placeholder-write') {
      frontmatter.air_id = airId;
    }
  }
