...
/**
 * Read options for authority CAS gets.
 *
 * Netlify Functions Lambda-compat (`connectLambda`) has no `uncachedEdgeURL`, so
 * `consistency: 'strong'` throws `BlobsConsistencyError` and helper-access returns 503
 * while legacy grant list (eventual `get`) still succeeds. Match gateway-auth /
 * CONNECT-GITHUB guidance: eventual on Lambda; strong only on persistent hosts.
 *
 * CAS write safety still comes from `getWithMetadata` etags + `onlyIfMatch` / `onlyIfNew`.
 *
 * @returns {{ type: 'text', consistency?: 'strong' }}
 */
export function authorityBlobGetOpts() {
  const onLambda = Boolean(process.env.NETLIFY || process.env.AWS_LAMBDA_FUNCTION_NAME);
  return onLambda
    ? Object.freeze({ type: 'text' })
    : Object.freeze({ type: 'text', consistency: 'strong' });
}
...
export async function materializeAuditOutbox(envelope, cas) {
  ...
  for (const { kind, record } of records) {
    ...
    if (cas && typeof cas.set === 'function') {
      const existing = await cas.get(auditKey, authorityBlobGetOpts()).catch(() => null);
      if (typeof existing === 'string' && existing.trim()) {
        ...
        if (write && write.modified === false) {
          const again = await cas.get(auditKey, authorityBlobGetOpts()).catch(() => null);
...
export function createDelegationAuthorityStore(opts) {
  ...
  async function readActiveEnvelope() {
    let markerRaw;
    try {
      markerRaw = await cas.get(markerKey, authorityBlobGetOpts());
    } catch {
    ...
    if (!markerRaw || (typeof markerRaw === 'string' && !markerRaw.trim())) {
      ...
      const got = await cas.getWithMetadata(envelopeKey, authorityBlobGetOpts()).catch(() => null);
      return {
        ok: true,
        envelope,
    ...
    let got;
    try {
      got = await cas.getWithMetadata(marker.envelope_key || envelopeKey, authorityBlobGetOpts());
    } catch {
    ...
    while (attempt < CAS_MAX_RETRIES) {
      ...
      try {
        ...
        if (!etag) {
          ...
          const fresh = await cas.getWithMetadata(envelopeKey, authorityBlobGetOpts()).catch(() => null);
          if (fresh && fresh.etag) {
            etag = fresh.etag;
          } else {
