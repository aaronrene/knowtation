...
/** @typedef {'user_owned' | 'org_owned' | 'delegate'} AgentKind */
/** @typedef {'active' | 'suspended' | 'revoked'} IdentityStatus */
/** @typedef {'advance_step' | 'complete_task' | 'propose_outcome' | 'invoke_tool' | 'mint_subgrant'} AuditAction */
/** @typedef {'local' | 'hosted' | 'hybrid'} ExecutionLocation */

const AGENT_KINDS = new Set(['user_owned', 'org_owned', 'delegate']);
...
export function validateAgentIdentityRecord(record) {
  ...
  if (record.schema !== AGENT_IDENTITY_SCHEMA) return { ok: false, error: 'schema mismatch' };
...
export function grantForClient(stored) {
  const { grant_bearer_hash: _b, ...grant } = stored;
  return grant;
}
 ...
 *   scopeCeiling?: Scope,
 *   ownerRef?: string,
 *   createProposal: (dataDir: string, input: object) => object,
 * }} input
 */
export function handleAgentIdentityRegisterProposeRequest(input) {
  const gate = checkDelegationGate(input.dataDir);
  ...
  const ownerRef =
    typeof input.ownerRef === 'string' && input.ownerRef.trim()
      ? input.ownerRef.trim()
      : principalRef;
 ...
 * }} input
 */
export function handleDelegationConsentProposeRequest(input) {
  const gate = checkDelegationGate(input.dataDir);
  if (!gate.ok) return gate;
 ...
 */
export function precheckApprovedDelegationProposal(dataDir, proposal) {
...
export function handleAgentIdentityListRequest(input) {
  ...
  let identities = vault && Array.isArray(vault.identities) ? vault.identities : [];
