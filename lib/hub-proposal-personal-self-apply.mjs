/**
 * Scooling personal self-apply class — HOSTED-WRITE-EVAL §HWE.3 + SEC-SEAM-1 +
 * FINISH-COMPLETE-APPLY T5 + FLOW-WRITE-LIVE §FWL.4.
 *
 ...
 * Tasks/Media under §FCA.4, or Flow authoring under §FWL.4.1), without a Hub
 * evaluation hop. Not a global member-approve grant.
 *
 ...
 * (T5) admits Tasks/Media fingerprints; FLOW-WRITE-LIVE-KN-b widens T5 for
 * Wave 1 Flow authoring (`new`|`edit`|`import`). Delegation stays unconditional
 * SELF_APPLY_DELEGATION_REFUSED; flow_capture stays SELF_APPLY_NOT_ADMITTED.
 *
 ...
 * @see ../scooling/docs/FLOW-WRITE-LIVE-FREEZE.md §FWL.4
...
import { normalizeCanisterProposalForFlowPrecheck } from './flow/flow-hosted-proposal.mjs';
import { normalizeCanisterProposalForCapturePrecheck } from './flow/flow-capture-hosted-proposal.mjs';
import { normalizeCanisterProposalForMediaPrecheck } from './attachments/media-hosted-proposal.mjs';
import { TASK_PROPOSAL_SOURCE } from './task/task-write.mjs';
import { PATH_PROPOSAL_SOURCE } from './path/path-write.mjs';
...
/** FLOW-WRITE-LIVE §FWL.4.1 — Flow mirror path (approve-time admission) */
export const SCOOLING_FLOW_MIRROR_PATH_RE =
  /^meta\/flows\/[A-Za-z0-9._:-]{1,128}\.md$/;

/** Closed Flow kind allowlist (§FWL.4.1) — exact FlowProposeKind; no default. */
export const ADMITTED_FLOW_PROPOSAL_KINDS = Object.freeze(['new', 'edit', 'import']);

const ADMITTED_FLOW_KIND_SET = new Set(ADMITTED_FLOW_PROPOSAL_KINDS);
...
export function matchesScoolingMediaFingerprint(proposal, opts = {}) {
  ...
  // SEC-SEAM-MEDIA SM-C8: hosted canister rows carry source/media_meta in
  // frontmatter — normalize first so T5 fingerprint evaluation stays possible.
  const effective = normalizeCanisterProposalForMediaPrecheck(proposal) ?? proposal;
  if (effective.source !== MEDIA_PROPOSAL_SOURCE) return false;

  const body = parseProposalBodyObject(effective);
  const kindFromMeta =
    effective.media_meta &&
    typeof effective.media_meta === 'object' &&
    typeof /** @type {{ proposal_kind?: string }} */ (effective.media_meta).proposal_kind === 'string'
      ? String(/** @type {{ proposal_kind: string }} */ (effective.media_meta).proposal_kind).trim()
      : '';
  const kindFromBody =
  ...
  const pathOk = opts.allowPendingPath
    ? proposalMirrorPathShapeOk(String(effective.path ?? ''), SCOOLING_MEDIA_PROPOSAL_PATH_RE)
    : proposalMirrorPathAdmitted(
        String(effective.path ?? ''),
        SCOOLING_MEDIA_PROPOSAL_PATH_RE,
        typeof effective.proposal_id === 'string' ? effective.proposal_id : null,
      );
  if (!pathOk) return false;

  const externalRef = String(effective.external_ref ?? '').trim();
...
/**
 * Scope for Flow proposals (§FWL.4.1): frontmatter.scope → body.flow.scope.
 * @param {Record<string, unknown>|null|undefined} proposal
 * @returns {string}
 */
export function extractFlowScope(proposal) {
  if (!proposal || typeof proposal !== 'object') return '';
  const fm = proposal.frontmatter;
  if (fm && typeof fm === 'object' && !Array.isArray(fm)) {
    const s = /** @type {Record<string, unknown>} */ (fm).scope;
    if (typeof s === 'string' && s.trim()) return s.trim();
  }
  const body = parseProposalBodyObject(proposal);
  if (body) {
    ...
    if (flow && typeof flow === 'object' && !Array.isArray(flow)) {
      const s = /** @type {Record<string, unknown>} */ (flow).scope;
      if (typeof s === 'string' && s.trim()) return s.trim();
    }
  }
  return '';
...
/**
 * FLOW-WRITE-LIVE §FWL.4.1 — Flow authoring fingerprint.
 * Missing/empty `flow_meta.kind` is not admitted (do not mirror create-time
 * `kind || 'new'` default in proposals-store).
 *
 * @param {Record<string, unknown>|null|undefined} proposal
 * @returns {boolean}
 */
export function matchesScoolingFlowFingerprint(proposal) {
  if (!proposal || typeof proposal !== 'object') return false;
  if (isDelegationSurfaceProposal(proposal)) return false;

  // Hosted canister rows carry source/flow_meta in frontmatter — normalize first.
  const effective = normalizeCanisterProposalForFlowPrecheck(proposal) ?? proposal;
  if (effective.source !== FLOW_PROPOSAL_SOURCE) return false;

  const meta = effective.flow_meta;
  if (!meta || typeof meta !== 'object' || Array.isArray(meta)) return false;
  const kindRaw = /** @type {Record<string, unknown>} */ (meta).kind;
  // Exact allowlist only — empty / missing / whitespace ⇒ refuse (no default-to-new).
  if (typeof kindRaw !== 'string' || !kindRaw.trim()) return false;
  const kind = kindRaw.trim();
  if (!ADMITTED_FLOW_KIND_SET.has(kind)) return false;

  const notePath = String(effective.path ?? '').trim().replace(/^\/+/, '');
  if (!SCOOLING_FLOW_MIRROR_PATH_RE.test(notePath)) return false;

  const externalRef = String(effective.external_ref ?? '').trim();
  if (!SCOOLING_FLOW_EXTERNAL_REF_RE.test(externalRef)) return false;

  if (extractFlowScope(effective) !== 'personal') return false;

  return true;
}

/**
 * T5 positive admission for Tasks/Media/Flow (author used for task_assign self-assign).
 * @param {Record<string, unknown>|null|undefined} proposal
...
export function isAdmittedSeamSelfApplyFingerprint(proposal, authorActorId, opts = {}) {
  if (matchesScoolingFlowFingerprint(proposal)) return true;
...
export function roleEligibleForPersonalSelfApply(role, actor = {}) {
  ...
  if (String(actor.tokenType || '').trim() === 'agent_access') return false;
...
export function isSeamSurfaceProposal(proposal) {
  try {
    ...
    if (normalizeCanisterProposalForFlowPrecheck(proposal) != null) return true;
    // Hosted canister capture rows carry source/capture_meta in frontmatter only.
    // Normalize so Wave 2 refuse-all (FCL-C3) still hits SELF_APPLY_NOT_ADMITTED.
    if (normalizeCanisterProposalForCapturePrecheck(proposal) != null) return true;
    // SEC-SEAM-MEDIA SM-C2 (S3.0/S3.1 same change): hosted media rows carry markers
    // only in frontmatter — the SAME normalize is the gateway media apply trigger.
    if (normalizeCanisterProposalForMediaPrecheck(proposal) != null) return true;
    ...
    if (source === PATH_PROPOSAL_SOURCE) return true;
    if (proposal.review_queue === 'learning-path') return true;
    if (
      typeof proposal.path === 'string' &&
...
/**
 * SEC-SEAM-1 / S6 + FINISH-COMPLETE-APPLY T5 + FLOW-WRITE-LIVE — total refusal reason.
 * Precedence is frozen in S6.1. Step 11 admits Tasks/Media/Flow fingerprints;
 * flow_capture stays out; Delegation refused earlier.
 *
 * @param {{
...
export function personalSelfApplyRefusalReason(opts) {
  ...
  if (seam) {
    ...
    // 11 — T5 admission (Tasks/Media/Flow fingerprints; flow_capture stays out)
 ...
 * Review-tray: fingerprint only (unchanged). Tasks/Media/Flow: fingerprint + session/author gates.
 *
 ...
 * T5: also stamps admitted Task/Media/Flow fingerprints when sessionBound + author hold.
 *
