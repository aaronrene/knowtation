...
export function parseMuseConfigFromEnv(env = process.env) {
  ...
  const apiKey =
    env.MUSE_API_KEY != null && String(env.MUSE_API_KEY).trim()
      ...
      : '';
  const lineageParsed = parseInt(String(env.MUSE_LINEAGE_TIMEOUT_MS || ''), 10);
  const lineageTimeoutMs = Math.min(
    60_000,
    Math.max(
      1000,
      Number.isFinite(lineageParsed) && lineageParsed > 0 ? lineageParsed : DEFAULT_LINEAGE_TIMEOUT_MS,
    ),
  );
  const proxyParsed = parseInt(String(env.MUSE_PROXY_MAX_BYTES || ''), 10);
  const proxyMaxBytes = Math.min(
    10 * 1024 * 1024,
    Math.max(
      1024,
      Number.isFinite(proxyParsed) && proxyParsed > 0 ? proxyParsed : DEFAULT_PROXY_MAX_BYTES,
    ),
  );
