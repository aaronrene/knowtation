 ...
 *
 * `liveStateId` is the fingerprint of the **canister GET** frontmatter+body (same
 * inputs Hub clients hash for `base_state_id`). It must NOT be recomputed from
 * `readNote` after yaml-stage: `parseFrontmatterAndBody` `trimEnd`s the body, so
 * trailing newlines present in the canister payload would flip `kn1_` and false-
 * refuse with `MEDIA_LINEAGE_CONFLICT`.
 *
 * @param {{ canisterUrl: string, headers: Record<string, string>, notePath: string }} opts
 * @returns {Promise<{ vaultPath: string, staged: boolean, cleanup: () => void, liveStateId?: string, canisterFrontmatter?: Record<string, unknown>, canisterBody?: string } | { error: { ok: false, status: number, code: string, error: string } }>}
 */
export async function stageCanisterNoteToTempVault(opts) {
  ...
  const liveStateId = noteStateIdFromParts(fetched.frontmatter, fetched.body);
  return {
    vaultPath,
    staged: true,
    cleanup,
    liveStateId,
    canisterFrontmatter: fetched.frontmatter,
    canisterBody: fetched.body,
  };
}
...
export async function applyApprovedMediaProposalFromCanister(opts) {
  ...
  if (proposalKind === 'media_attach') {
    ...
    try {
      ...
      // Prefer canister GET body/frontmatter so we do not normalize trailing newlines
      // via readNote trimEnd during the write-back.
      const before =
        staged.canisterFrontmatter != null && typeof staged.canisterBody === 'string'
          ? { frontmatter: staged.canisterFrontmatter, body: staged.canisterBody }
          : readNote(staged.vaultPath, precheck.notePath);
      reconcileApprovedMediaProposal(opts.dataDir, precheck);
