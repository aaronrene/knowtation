...
export const ATTACHMENT_ID_RE = /^att_(file|mist|url)_[a-f0-9]{32}$/;
...
export const ATTACHMENT_SOURCES = /** @type {const} */ (['vault_file', 'mist_blob', 'embedded_url']);
...
export function deriveAllAttachments(vaultPath, dataDir, vaultId, options = {}) {
  ...
  for (const row of index.values()) {
    ...
    const scope =
      linkedRefs.length === 0 && row.source === 'vault_file'
        ? 'personal'
...
function isAttachmentNoteVisible(attachment, ctx) {
  if (attachment.linked_note_refs.length === 0) {
    return attachment.source === 'vault_file';
  }
