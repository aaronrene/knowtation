...
export async function fetchOperatorUserIndexPage(baseUrl, operatorKey, cursor = '', limit = 200) {
  ...
  if (!r.ok) {
    ...
    const body = t.slice(0, 200);
    if (r.status === 401) {
      throw new Error(
        `operator user index 401 ${body} — X-Operator-Export-Key does not match hub stable secret. Set GitHub secret KNOWTATION_OPERATOR_EXPORT_KEY to the exact same string passed to admin_set_operator_export_secret (dfx canister call hub ...).`,
      );
    }
    if (r.status === 503) {
      throw new Error(
        `operator user index 503 ${body} — Operator export not configured on canister. Run admin_set_operator_export_secret as a controller, then retry.`,
      );
    }
    throw new Error(`operator user index ${r.status} ${body}`);
  }
  return r.json();
}
