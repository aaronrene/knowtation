...
export function noteRecordFromExportPayload(n) {
  ...
  // Phase 12 — blockchain frontmatter fields
  const network = fm.network != null ? String(fm.network).trim() : undefined;
  const wallet_address = fm.wallet_address != null ? String(fm.wallet_address).trim() : undefined;
  const tx_hash = fm.tx_hash != null ? String(fm.tx_hash).trim() : undefined;
  const payment_status = fm.payment_status != null ? String(fm.payment_status).trim() : undefined;
  return {
    path,
    body,
    frontmatter: fm,
...
function frontmatterSearchStrings(fm) {
  ...
  const keys = [
    'title', 'intent', 'source', 'proposal_id', 'target_path', 'description', 'summary',
    // Phase 12 — blockchain fields included in keyword match
    'network', 'wallet_address', 'tx_hash', 'payment_status', 'currency', 'direction', 'air_id',
  ];
  const parts = [];
  for (const k of keys) {
    const v = fm[k];
