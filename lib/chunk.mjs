...
/**
 * Build chunks for one note. Each chunk has text and metadata (path, project, tags, date; optional causal_chain_id, entity, episode_id).
 * @param {{ body: string, path: string, project?: string, tags?: string[], date?: string, causal_chain_id?: string, entity?: string[], episode_id?: string }} note - From vault.readNote
 * @param {{ chunkSize?: number, chunkOverlap?: number }} options
 * @returns {{ id: string, text: string, path: string, project?: string, tags: string[], date?: string, causal_chain_id?: string, entity: string[], episode_id?: string }[]}
 */
export function chunkNote(note, options = {}) {
  ...
  const causal_chain_id = note.causal_chain_id || undefined;
  const entity = Array.isArray(note.entity) ? note.entity : [];
  const episode_id = note.episode_id || undefined;
