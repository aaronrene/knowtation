...
/**
 * Build vault-relative path, body, and inbox frontmatter for a fast capture (local disk or hosted canister write).
 *
 * @param {string} text
 * @param {{ source?: string, project?: string, tags?: string[] }} options
 * @param {Date} [now] - defaults to `new Date()`; inject in tests for stable paths
 * @returns {{ path: string, body: string, frontmatter: Record<string, unknown> }}
 */
export function buildCaptureInboxWritePayload(text, options = {}, now = new Date()) {
 ...
 */
export async function runCaptureInbox(text, options = {}) {
  const config = loadConfig();
  const { path, body, frontmatter } = buildCaptureInboxWritePayload(text, options);
  return writeNote(config.vault_path, path, {
    body,
    frontmatter,
  });
}
