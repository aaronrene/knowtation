...
/**
 * S3 key prefix for backup uploads; always ends with `/`.
 * If `KNOWTATION_CANISTER_BACKUP_S3_PREFIX` is unset, empty, or whitespace only, uses
 * `knowtation-canister-backups/`. (Plain `??` in callers would not treat `""` as missing,
 * which made GitHub Actions pass-through empty vars upload to the wrong key and IAM deny.)
 * @param {NodeJS.ProcessEnv} env
 * @returns {string}
 */
export function resolveBackupS3Prefix(env) {
  const raw = (env.KNOWTATION_CANISTER_BACKUP_S3_PREFIX ?? '').trim();
  const base = raw === '' ? 'knowtation-canister-backups' : raw.replace(/\/+$/, '');
  return `${base}/`;
}
