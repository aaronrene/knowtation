...
import { filterHitsByContentScope } from './approval-log.mjs';
...
export async function runSearch(query, options = {}, configOverride = null) {
  ...
  const scopeForFetch = options.content_scope;
  const searchLimit =
    countOnly ? 1000 : scopeForFetch && scopeForFetch !== 'all' ? Math.min(300, Math.max(limit * 6, limit)) : limit;
  ...
  hits = filterHitsByContentScope(hits, options.content_scope || 'all');
  if (countOnly) {
