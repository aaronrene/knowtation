...
/** @typedef {{ project?: string, outputDir?: string, tags?: string[], dryRun?: boolean }} ImportOptions */
...
export async function runImport(sourceType, input, options = {}) {
  ...
  const ctx = { vaultPath, outputBase, project, tags: Array.isArray(tags) ? tags : String(tags).split(',').map((t) => t.trim()).filter(Boolean), dryRun };
  const importers = {
