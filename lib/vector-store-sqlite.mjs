 ...
 *   vector_store: string,
 *   data_dir: string,
 *   allow_dimension_migration?: boolean,
 * }} config - `allow_dimension_migration: true` lets `ensureCollection` drop + recreate
 *   the table when the persisted dimension differs from the requested dimension. Bridge
 *   sets this because it owns the data lifecycle (downloads from blob, indexes, uploads
 *   to blob); a provider switch (e.g. OpenAI 1536 → DeepInfra 1024) can only resolve via
 *   a full re-embed. CLI leaves it OFF so an accidental EMBEDDING_PROVIDER swap surfaces
 *   loudly instead of silently corrupting search.
...
export function createSqliteVectorStore(config) {
  ...
  sqliteVec.load(db);
  const allowDimensionMigration = config?.allow_dimension_migration === true;
  ...
  return {
    ...
    async ensureCollection(dimension) {
      ...
      if (tableInfo && tableInfo.sql) {
        ...
        const dimMismatch = existingDim != null && existingDim !== dimension;
        if (dimMismatch && !allowDimensionMigration) {
          throw new Error(
        ...
        // tables cannot be ALTERed, so we drop + recreate.
        // Phase 19a (`fix/bridge-vector-store-dim-and-hash-model-binding`): when
        // `allow_dimension_migration` is set (bridge only), a dimension change also triggers
        // drop + recreate. Vectors at the old dim are mathematically incompatible with queries
        // at the new dim — there is no "migrate in place" option, only re-embed.
        const hasVaultId = /\bvault_id\b/i.test(tableInfo.sql);
        const hasContentHash = /\bcontent_hash\b/i.test(tableInfo.sql);
        const needsRecreate = dimMismatch || !hasVaultId || !hasContentHash;
        if (needsRecreate) {
          if (dimMismatch) {
            console.warn(
              `[vector-store-sqlite] dimension migration: dropping ${TABLE_NAME} ` +
                `(existing dim ${existingDim} → requested dim ${dimension}). ` +
                'All vectors for all vaults in this DB will be removed and must be re-embedded. ' +
                'This is expected after switching EMBEDDING_PROVIDER or EMBEDDING_MODEL.',
            );
          }
