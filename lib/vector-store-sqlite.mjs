  ...
  return {
    ...
    async ensureCollection(dimension) {
      ...
      if (tableInfo && tableInfo.sql) {
        ...
        if (!/\bvault_id\b/i.test(tableInfo.sql)) {
    ...
    async upsert(points) {
      ...
      const run = db.transaction((pts) => {
        for (const p of pts) {
          ...
          db.exec(
            `INSERT INTO ${TABLE_NAME} (id, embedding, path, project, date, causal_chain_id, episode_id, vault_id, tags, entity, chunk_text) VALUES (${id}, '${embeddingStr}', ${esc(p.path)}, ${esc(p.project)}, ${esc(p.date)}, ${esc(p.causal_chain_id)}, ${esc(p.episode_id)}, ${vaultStr}, ${esc(tagsStr)}, ${esc(entityStr)}, ${esc(p.text)})`
          );
        }
      });
      run(points);
    ...
    async search(queryVector, options = {}) {
      ...
      if (pathPrefixFilter) {
        fetchLimit = Math.min(Math.max(limit * 100, 2000), 10000);
      } else if (needsWideFetch) {
        fetchLimit = Math.min(vaultScoped ? Math.max(limit * 25, 400) : limit * 3, vaultScoped ? 2000 : 300);
      }
