 ...
 *
 * Phase 6 migration (D6.6.2): runDiscoverPass routes insight persistence through
 * DerivedArtifactWriter. The direct mm.store('insight', ...) call is removed.
 *
...
import { createDerivedArtifactWriter } from './companion-artifact-writer.mjs';
import { buildConvenienceProvenance } from './companion-provenance-validator.mjs';
 ...
 *
 * Phase 6 migration (D6.6.2): insight persistence now routes through the
 * DerivedArtifactWriter. The direct mm.store('insight', ...) call is removed.
 * For the convenience/self-partition default the behavior is unchanged.
 *
 ...
 *   dryRun?: boolean,
 *   llmFn?: Function,
 *   mm?: import('./memory.mjs').MemoryManager,
 *   writer?: import('./companion-artifact-writer.mjs').DerivedArtifactWriter,
 *   writerContext?: import('./companion-artifact-writer.mjs').WriteContext,
 *   generatedBy?: string,
 *   model?: string,
 *   modelVersion?: string,
...
export async function runDiscoverPass(config, consolidations, opts = {}) {
  ...
  // Phase 6 (D6.6.2): route through DerivedArtifactWriter — no direct mm.store('insight', ...).
  const mm = opts.mm ?? createMemoryManager(config);

  // Build writer + provenance for convenience/self-partition default.
  const writer = opts.writer ?? createDerivedArtifactWriter({
    writeNoteFn: () => { /* insight artifacts don't use writeNoteFn */ },
    vaultPath: config.vault_path || '.',
    mm,
    vaultRegistryAvailable: false,
  });

  const context = opts.writerContext ?? {
    lane: 'local',
  ...
  // source_event_id: collect consolidation event ids if available; fall back to timestamp slug
  const sourceEventIds = consolidations
    .map((c) => c.id)
    .filter((id) => typeof id === 'string' && id);
  const sourceEventId = sourceEventIds.length > 0
    ? sourceEventIds
    : [`discover-${Date.now()}`];
  ...
  const artifact = { connections, contradictions, open_questions, topic_count };

  await writer.write(artifact, provenance, context);
