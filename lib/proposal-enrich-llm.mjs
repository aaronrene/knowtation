...
- suggested_frontmatter: only use keys from the allow-list. Prefer returning every field that is clearly grounded in the note path, intent, headings, body text, or explicit references. Omit only fields that are genuinely unsupported by the content.
...
Extract metadata from the actual content, not just from any existing frontmatter-like wording.

Field guidance:
- title: infer a strong note title from the heading/body when possible.
- project: infer from the note path, named initiative, or clearly repeated project framing.
- tags: include concrete topic tags from the body, not generic filler.
- date / updated: include only if the document states a concrete date or time anchor.
- source / source_id: include only if the text names a source system, document, URL, export id, ticket id, etc.
- intent: infer the operational purpose if the body makes it clearer than the provided intent.
- follows: include prior note paths only when the body/path clearly references an earlier note or continuation.
- causal_chain_id: infer when the document describes a named chain of events, incident thread, rollout, investigation, or dependency chain.
- entity: extract the main people, teams, companies, products, systems, repos, or domains discussed.
- episode_id: infer when this note belongs to a clearly bounded episode, milestone, launch, sprint, outage, or test run.
- summarizes / summarizes_range: infer only when the note explicitly summarizes another note, folder, period, or range.
- state_snapshot: true only when the note is clearly capturing status/state at a moment in time.

Prioritize temporal, causal, entity, and relationship metadata when the content supports it. Do not stop at title/project/tags if richer grounded metadata is present.
---
${body}`;
  return { system, user };
}
