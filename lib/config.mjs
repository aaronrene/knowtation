...
export function loadConfig(cwd = projectRoot) {
  ...
  /**
   * Flow workspace identity (Phase 7A): the local CLI/MCP operator's authorized Flow scopes.
   * This is the "local config identity" channel the Flow store/projection surfaces resolve scope
   * through (FLOW-STORE-CONTRACT-7A-10 §4) — the CLI passes it as `visibleScopes` into the store;
   * no CLI flag can widen it. Deny-by-default: absent ⇒ `personal` only (resolved downstream in
   * `flow-scope.mjs`). Values are validated against the canonical scope set there; here we only
   * surface a clean string array so a malformed config can never inject a non-string scope.
   */
  const flowRaw = config.flow && typeof config.flow === 'object' ? config.flow : null;
  const flowVisibleScopes =
    flowRaw && Array.isArray(flowRaw.visible_scopes)
      ? flowRaw.visible_scopes.filter((s) => typeof s === 'string' && s.trim() !== '')
      : undefined;

  const flowCaptureRaw =
    flowRaw?.capture && typeof flowRaw.capture === 'object' ? flowRaw.capture : null;
  const flowCapture = flowCaptureRaw
    ? {
        enabled: flowCaptureRaw.enabled !== false,
        session_extraction_opt_in: flowCaptureRaw.session_extraction_opt_in === true,
        classroom_minor_mode: flowCaptureRaw.classroom_minor_mode === true,
        min_confidence_floor:
          flowCaptureRaw.min_confidence_floor === 'low' ||
          flowCaptureRaw.min_confidence_floor === 'high'
            ? flowCaptureRaw.min_confidence_floor
            : 'medium',
      }
    : undefined;
  ...
  return {
    ...
    llm: loadLlmConfig(config.llm),
    flow:
      (flowVisibleScopes && flowVisibleScopes.length > 0) || flowCapture
        ? {
            ...(flowVisibleScopes && flowVisibleScopes.length > 0
              ? { visible_scopes: flowVisibleScopes }
              : {}),
            ...(flowCapture ? { capture: flowCapture } : {}),
          }
        : undefined,
...
/** Chat providers selectable for the `completeChat` path (Hub Settings UI + config llm.provider). */
export const CHAT_PROVIDERS = Object.freeze(['deepinfra', 'openrouter', 'openai', 'anthropic', 'ollama']);

/**
 * Validate a chat-provider value submitted from the Hub Settings UI before it is persisted.
 *
 * Security boundary: the persisted provider drives where note text is sent (privacy) and which
 * account is billed. Only an empty string ("" → auto-detect) or an exact whitelisted provider is
 * accepted; anything else is rejected so a malformed/hostile value can never be written to
 * config/local.yaml or reach completeChat.
 *
 * @param {unknown} value
 * @returns {{ ok: true, provider: string } | { ok: false, error: string }}
 */
export function normalizeChatProviderInput(value) {
  if (value == null || value === '') return { ok: true, provider: '' };
  if (typeof value !== 'string') {
    return { ok: false, error: 'provider must be a string' };
  }
  const v = value.trim().toLowerCase();
  if (v === '') return { ok: true, provider: '' };
  if (!CHAT_PROVIDERS.includes(v)) {
    return {
      ok: false,
      error: `provider must be one of: ${CHAT_PROVIDERS.join(', ')} (or empty for auto-detect)`,
    };
  }
  return { ok: true, provider: v };
}

/**
 * Resolve the top-level `llm` block (the `completeChat` path) from local.yaml.
 *
 * Distinct from `daemon.llm` (consolidation/Discovery). `provider` selects the chat lane used by
 * MCP summarize and Hub proposal LLM jobs; invalid values are dropped (treated as auto-detect) so a
 * malformed config can never force an unknown provider. The env var `KNOWTATION_CHAT_PROVIDER` still
 * takes precedence at resolution time (see lib/llm-complete.mjs).
 *
 * @param {object|undefined} raw — the `llm` key from the YAML file
 * @returns {{ provider: string, openai_chat_model: string|undefined, anthropic_chat_model: string|undefined, deepinfra_chat_model: string|undefined, openrouter_chat_model: string|undefined, ollama_chat_model: string|undefined }}
 */
export function loadLlmConfig(raw) {
  const l = raw && typeof raw === 'object' ? raw : {};
  const providerRaw = String(l.provider || '').trim().toLowerCase();
  const provider = CHAT_PROVIDERS.includes(providerRaw) ? providerRaw : '';
  return {
    provider,
    openai_chat_model: l.openai_chat_model || undefined,
    anthropic_chat_model: l.anthropic_chat_model || undefined,
    deepinfra_chat_model: l.deepinfra_chat_model || undefined,
    openrouter_chat_model: l.openrouter_chat_model || undefined,
    ollama_chat_model: l.ollama_chat_model || undefined,
  };
}
