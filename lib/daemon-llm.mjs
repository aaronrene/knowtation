...
/**
 * Build a merged config for a delegated completeChat call.
 *
 * For the explicit `anthropic` and `ollama` daemon providers this pins
 * `llm.provider` so completeChat routes to exactly that provider, independent of any
 * globally-configured chat provider (`config.llm.provider` from the Hub Settings UI) — the
 * daemon's provider choice must always win for its own passes. The model field is also
 * overridden when `daemon.llm.model` is set.
 *
 * For the null/openai delegate path the provider is intentionally left unset so completeChat
 * applies its normal precedence (env → config.llm.provider → auto-detection).
 *
...
export function buildDelegateConfig(config, { provider, model }) {
  ...
  if (provider === 'anthropic') {
    llmPatch.provider = 'anthropic';
    if (model) llmPatch.anthropic_chat_model = model;
  } else if (provider === 'ollama') {
    llmPatch.provider = 'ollama';
    if (model) llmPatch.ollama_chat_model = model;
  } else if (model) {
    llmPatch.openai_chat_model = model;
  }
  if (Object.keys(llmPatch).length === 0) return config;
