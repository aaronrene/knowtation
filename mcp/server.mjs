#!/usr/bin/env node
/**
 * Knowtation MCP Server — Phase 9.
 * Exposes search, get_note, list_notes, index, write, export, import as MCP tools.
 * Stdio transport; single backend (reuses lib/* logic).
 * Run: node mcp/server.mjs or knowtation mcp
 */
...
server.registerTool(
  ...
  async (args) => {
    try {
      const config = loadConfig();
      let paths = [];
      const looksLikePath = !args.path_or_query.includes(' ') && (args.path_or_query.endsWith('.md') || args.path_or_query.includes('/'));
      if (looksLikePath) {
        try {
...
server.registerTool(
  'import',
  {
    description: 'Import from external source (chatgpt-export, claude-export, mem0-export, mif, markdown, audio, video).',
    inputSchema: {
      source_type: z
