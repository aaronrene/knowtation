...
- **LLM proposal helpers:** **Review hints** on hosted run on this **gateway** after proposal create when **`KNOWTATION_HUB_PROPOSAL_REVIEW_HINTS=1`** (`hub/gateway/proposal-review-hints-async.mjs`). **Enrich** still runs only on the **Node Hub** (`hub/server.mjs`); hosted Enrich needs a future gateway/canister path. See **[docs/HUB-PROPOSAL-LLM-FEATURES.md](../../docs/HUB-PROPOSAL-LLM-FEATURES.md)**.
