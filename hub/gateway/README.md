...
- **POST /api/v1/auth/establish-refresh** — Exchange a valid human `type:session` Bearer JWT for a durable refresh token (JSON body + HttpOnly cookie). Used when OAuth redirect cannot attach `Set-Cookie` (Netlify); Hub UI and `scripts/hub-session-refresh.mjs --save-access-token` call this after login.
