...
- **LLM proposal helpers:** **Review hints** on hosted run on this **gateway** after proposal create when **`KNOWTATION_HUB_PROPOSAL_REVIEW_HINTS=1`** (`hub/gateway/proposal-review-hints-async.mjs`). **Enrich** on hosted runs on the gateway when **`KNOWTATION_HUB_PROPOSAL_ENRICH=1`** (`hub/gateway/proposal-enrich-hosted.mjs`): **`POST /api/v1/proposals/:id/enrich`** calls the chat backend then updates the canister. Deploy the **hub** canister from this repo first (stable migration V4 adds enrich fields). See **[docs/HUB-PROPOSAL-LLM-FEATURES.md](../../docs/HUB-PROPOSAL-LLM-FEATURES.md)**.
