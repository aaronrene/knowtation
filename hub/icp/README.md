...
- **After mainnet has run V1→current:** `Migration.migration` is an **identity** on `StableStorage` (`{ var storage = old.storage }`). Future upgrades that change `StableStorage` must replace this hook with a real mapper from the **previous** on-chain shape to the new one.
