...
Both run **migration shape checks** (`npm run canister:verify-migration`), **`npm test`**, and **`dfx build hub --network ic`** (matches [canister_ids.json](./canister_ids.json); plain `dfx build hub` targets **local** and fails with “Cannot find canister id” until you run `dfx canister create hub` on a local replica). Override: **`DFX_PREFLIGHT_NETWORK=local`** after local create. Optional JSON backup: set `KNOWTATION_CANISTER_BACKUP_USER_ID` (and optionally `KNOWTATION_CANISTER_URL`; omitted URL is derived from `canister_ids.json`). Preflight delegates to [`scripts/canister-export-backup.sh`](../../scripts/canister-export-backup.sh); you can also run **`npm run canister:export-backup`**. Exports: `backups/canister-export-<vault>-<UTC>.json` (gitignored). **Scheduled GitHub backup:** see [DEPLOY-HOSTED.md](../../docs/DEPLOY-HOSTED.md) §6. If `dfx` crashes with **ColorOutOfRange**, use **`SKIP_DFX_BUILD=1`** after you have built successfully elsewhere, or upgrade `dfx`.
