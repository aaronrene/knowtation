...
if [[ "${SKIP_DFX_BUILD:-}" == "1" ]]; then
  ...
  WASM_LOCAL="$ICP_DIR/.dfx/local/canisters/hub/hub.wasm"
  if [[ -f "$WASM_LOCAL" ]]; then
    echo "    Built wasm: $WASM_LOCAL ($(wc -c < "$WASM_LOCAL" | tr -d ' ') bytes)"
  else
    echo "WARN: Expected wasm not found at $WASM_LOCAL (dfx output layout may differ)."
  fi
fi
